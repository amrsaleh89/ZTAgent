...
// -----------------------------------------
//
// Source: D2_Card_ServiceC5.java
// Date  : 21 Mar 2017 10:17:55 PST
 ...
 * Automatically generated load test program.
 */
public class D2_Card_ServiceC5 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_2 = "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:50.0) Gecko/20100101 Firefox/50.0";
	...
	private static final String EXTERNAL_PARAM_FILE = "D2_Card_ServiceC5ExtParams.dat";     // input file name for very large parameter values
	...
	private static String CookieName = "DIBSID";                                 // var declaration from web admin var handler: scope = global
	...
	private String dibsmd5 = null;                       // var declaration from web admin var handler: scope = per loop
	private String token = null;                         // var declaration from web admin var handler: scope = per loop
	private String DIBSID1 = null;                       // var declaration from web admin var handler: scope = per loop
	 ...
	 */
	public D2_Card_ServiceC5(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log("# Page #1: POST");
		log("# -------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #1: POST", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "payment.architrade.com";
		int    requestPort0001 = 443;
		String requestFile0001 = "/paymentweb/start.action";
		String requestContent0001 = 
				"accepturl=https%3A%2F%2Fpayment.architrade.com" +
				"&amount=100" +
				"&calcmd5=0" +
				"&cancelurl=https%3A%2F%2Fpayment.architrade.com" +
				"&color=blue" +
				"&currency=208" +
				"&lang=da" +
				"&merchant=90221119" +
				"&orderid=DTEST" +
				"&ordertext=DIBS+TEST+%C3%A6%C3%B8%C3%A5+%C3%A4%C3%B6" +
				"&paywin=new" +
				"&test=yes";
		String requestHeader0001 = "POST " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: payment.architrade.com\r\n" + 
				"Accept: */*\r\n" + 
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 7838
		...
		if (!urlCallPassed)
		{
			...
			((GenericPluginInterface) plugin0001).setInputParameter(0, CookieName);		// set plug-in input parameter
			...
			DIBSID1 = (String) ((GenericPluginInterface) plugin0001).getOutputParameter(0);		// get plug-in output parameter
			log("<<< DIBSID1 = " + DIBSID1);
		...
		String requestHost0002 = "payment.architrade.com";
		int    requestPort0002 = 443;
		String requestFile0002 = "/paymentweb/paytype.action;dibsid=t31TFKY4-KNb7jVMgaapxV0veZznJble8DcBPiXR";
		requestFile0002 = Lib.replaceTextPattern(requestFile0002, "t31TFKY4-KNb7jVMgaapxV0veZznJble8DcBPiXR", DIBSID1, 0);		// search and replace request file fragment "t31TFKY4-KNb7jVMgaapxV0veZznJble8DcBPiXR" with variable 'DIBSID1' on all occurrences
		...
		testURL.replaceRequestHeaderFieldTextPattern("t31TFKY4-KNb7jVMgaapxV0veZznJble8DcBPiXR", DIBSID1);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 6743
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "Butikkens ordrenr");		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		{
			// extract var 'dibsmd5'
			dibsmd5 = htmlContentParser.getFormTable().getForm("paymentform").getFormItemValue("dibsmd5");		// recorded value = c15a12917aa36c6dcc53146c07cc745f
			log("<<< dibsmd5 = " + dibsmd5);
		}
		catch (Exception e) { dibsmd5 = null; log(e); }
		if (dibsmd5 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'dibsmd5' from html form parameter";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		{
			// extract var 'token'
			token = htmlContentParser.getFormTable().getForm("paymentform").getFormItemValue("token");		// recorded value = 3E8JI1XU1451M7J7T9DHELG5O42OMAZT
			log("<<< token = " + token);
		}
		...
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'token' from html form parameter";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		String requestHost0003 = "payment.architrade.com";
		int    requestPort0003 = 443;
		String requestFile0003 = "/paymentweb/auth.action";
		String requestContent0003 = 
				"struts.token.name=token" +
				"&token=" + token +		// recorded value = 3E8JI1XU1451M7J7T9DHELG5O42OMAZT
				"&accepturl=https%3A%2F%2Fpayment.architrade.com%3A443%2Fpaymentweb%2Freply.action" +
				"&cancelurl=https%3A%2F%2Fpayment.architrade.com%3A443%2Fpaymentweb%2Freply.action" +
				"&declineurl=https%3A%2F%2Fpayment.architrade.com%3A443%2Fpaymentweb%2Freply.action" +
				"&dibsmd5=" + dibsmd5 +		// recorded value = c15a12917aa36c6dcc53146c07cc745f
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4309
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "Betaling godkendt");		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0004 = "payment.architrade.com";
		int    requestPort0004 = 443;
		String requestFile0004 = "/paymentweb/smd.action";
		...
		String requestHost0005 = "payment.architrade.com";
		int    requestPort0005 = 443;
		String requestFile0005 = "/paymentweb/smd.action";
		String requestContent0005 = 
				"{\"params\":[\"User finished\",\"acceptform\"],\"method\":\"clearSession\",\"id\":1}";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 58
		// content test algorithm: none - no content check
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("D2_Card_ServiceC5", concurrentUsers);
		...
		{
			System.out.println("# global var CookieName = " + CookieName);
		}
		System.out.println();
		...
		performanceData.setInfoText("D2_Card_ServiceC5");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				D2_Card_ServiceC5 simulatedUser = new D2_Card_ServiceC5(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
