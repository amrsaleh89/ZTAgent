...
// -----------------------------------------
//
// Source: Q2E_USAmeriBank_Login__S.java
// Date  : 06 Jan 2017 18:44:38 ECT
 ...
 * Automatically generated load test program.
 */
public class Q2E_USAmeriBank_Login__S extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static final String USER_INPUT_FIELD_vTriggerSnapshot = "AQAQdlRyaWdnZXJTbmFwc2hvdAEAEFRyaWdnZXIgU25hcHNob3QBAAVmYWxzZQ@@";		// internal object-string representation of user input field 'vTriggerSnapshot'
	...
	private static final String EXTERNAL_PARAM_FILE = "Q2E_USAmeriBank_Login__SExtParams.dat";// input file name for very large parameter values
	...
	private static VarInputFileReader accounts_txt = null;           // input file accounts.txt, file scope: new line per loop

	private static String vTriggerSnapshot = null;                               // var declaration from web admin var handler: scope = global
	...
	private String password = null;                      // var declaration from web admin var handler: scope = per loop
	private String userId = null;                        // var declaration from web admin var handler: scope = per loop
	private String xcsrf_token = null;                   // var declaration from web admin var handler: scope = per loop
	private String userName = null;                      // var declaration from web admin var handler: scope = per loop
	
	private Object plugin0001 = null;                             // plug-in #1: "AppDynamicsIntegrationV4" - definition from web admin var handler: construction scope = per loop
	private static String plugin0001AppDynamicsIntegrationV4 = null;// internal - not used in this load test program, but used by the GUI - reflective information


	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public Q2E_USAmeriBank_Login__S()
	{
		super();
	}
	 ...
	 */
	public Q2E_USAmeriBank_Login__S(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		try
		{
			...
			{
				if (!accounts_txt.getNextLine())
				{
					abortByEofInputFile();
					return true;		// eof of input file - no error from web server
				}
				password = new String(accounts_txt.getLineVar(2));
				userId = new String(accounts_txt.getLineVar(1));
				userName = new String(accounts_txt.getLineVar(3));
			}
			log();
			log("<<< password = " + password);
			log("<<< userId = " + userId);
		...
		log("<<< vTriggerSnapshot = " + vTriggerSnapshot);
		...
		try
		{
			...
			Class pluginClass0 = Class.forName("AppDynamicsIntegrationV4");
		...
		log();
		log();
		log("# Page #1: Login Page");
		log("# -------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #1: Login Page", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		...
		log();
		log();
		log("# Page #2: Perform Login");
		log("# ----------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: Perform Login", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
		...
		log("# Page #3: Home");
		log("# -------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #3: Home", 0, 0, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "demo.q2ebanking.com";
		int    requestPort0001 = 443;
		String requestFile0001 = "/usameribankonline_test_2_927002/login.aspx";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: demo.q2ebanking.com\r\n" + 
		...
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = "Location: /usameribankonline_test_927002/Uux.aspx", recorded content size = 0
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302}, null, "Location: /usameribankonline_test_927002/Uux.aspx", null);		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		String requestHost0002 = "demo.q2ebanking.com";
		int    requestPort0002 = 443;
		String requestFile0002 = "/usameribankonline_test_2_927002/Uux.aspx";
		...
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0002);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4378
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "window.Q2_PRODUCTION_TAG = '4.1.1.4';");		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0003 = "cdn1.onlineaccess1.com";
		int    requestPort0003 = 443;
		String requestFile0003 = "/cdn/base/4.1.1.4/assets/vendorapp.js";
		...
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0003);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/X-JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 1139151
		// content test algorithm: none - no content check
		...
		String requestHost0004 = "cdn1.onlineaccess1.com";
		int    requestPort0004 = 443;
		String requestFile0004 = "/cdn/base/4.1.1.4/assets/app.css";
		...
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0004);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 110921
		// content test algorithm: none - no content check
		...
		String requestHost0005 = "cdn1.onlineaccess1.com";
		int    requestPort0005 = 443;
		String requestFile0005 = "/cdn/3249/4.1.1.4/000038/assets/theme-q2.css";
		...
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0005);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 184929
		// content test algorithm: none - no content check
		...
		String requestHost0006 = "cdn1.onlineaccess1.com";
		int    requestPort0006 = 443;
		String requestFile0006 = "/cdn/3249/4.1.1.4/000038/assets/themejs/theme-q2.js";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 1883
		// content test algorithm: none - no content check
		...
		String requestHost0007 = "cdn1.onlineaccess1.com";
		int    requestPort0007 = 443;
		String requestFile0007 = "/cdn/3249/4.1.1.4/000038/assets/resources/en-us.js";
		...
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0007);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/X-JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 265595
		// content test algorithm: none - no content check
		...
		String requestHost0008 = "cdn1.onlineaccess1.com";
		int    requestPort0008 = 443;
		String requestFile0008 = "/cdn/3249/4.1.1.4/000038/assets/theme-q2-blessed2.css" +
				"?z=1472155578103";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0008 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0008);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 362537
		// content test algorithm: none - no content check
		...
		String requestHost0009 = "cdn1.onlineaccess1.com";
		int    requestPort0009 = 443;
		String requestFile0009 = "/cdn/3249/4.1.1.4/000038/assets/theme-q2-blessed1.css" +
				"?z=1472155578103";
		...
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0009);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 428272
		// content test algorithm: none - no content check
		...
		String requestHost0010 = "cdn1.onlineaccess1.com";
		int    requestPort0010 = 443;
		String requestFile0010 = "/cdn/3249/4.1.1.4/000038/assets/fonts/OpenSans/OpenSans-Regular.woff";
		String requestHeader0010 = "GET " + requestFile0010 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Origin: https://demo.q2ebanking.com\r\n" + 
				"DNT: 1\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"Content-Length: 0\r\n" + 
				"\r\n";
		...
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0010);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/FONT-WOFF", header text fragment = [verification disabled], recorded content size = 24872
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/FONT-WOFF", null, null);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0011 = "cdn1.onlineaccess1.com";
		int    requestPort0011 = 443;
		String requestFile0011 = "/cdn/3249/4.1.1.4/000038/assets/overlays/overlays_desktop.js";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0011 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0011);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/X-JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 7426
		// content test algorithm: none - no content check
		...
		String requestHost0012 = "demo.q2ebanking.com";
		int    requestPort0012 = 443;
		String requestFile0012 = "/usameribankonline_test_2_927002/mobilews/preLogonUser";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0012 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0012);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 627
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0013 = "cdn1.onlineaccess1.com";
		int    requestPort0013 = 443;
		String requestFile0013 = "/cdn/3249/4.1.1.4/000038/assets/images/fdic_logo_small.png";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0013 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0013);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 1410
		// content test algorithm: none - no content check
		...
		String requestHost0014 = "cdn1.onlineaccess1.com";
		int    requestPort0014 = 443;
		String requestFile0014 = "/cdn/3249/4.1.1.4/000038/assets/images/logos/logo_large.png";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0014 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0014);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 42002
		// content test algorithm: none - no content check
		...
		String requestHost0015 = "cdn1.onlineaccess1.com";
		int    requestPort0015 = 443;
		String requestFile0015 = "/cdn/3249/4.1.1.4/000038/assets/fonts/OpenSans/OpenSans-Bold.woff";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0015 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0015);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/FONT-WOFF", header text fragment = [verification disabled], recorded content size = 25708
		// content test algorithm: none - no content check
		...
		String requestHost0016 = "cdn1.onlineaccess1.com";
		int    requestPort0016 = 443;
		String requestFile0016 = "/cdn/3249/4.1.1.4/000038/assets/fonts/OpenSans/OpenSans-Light.woff";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0016 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0016);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/FONT-WOFF", header text fragment = [verification disabled], recorded content size = 24192
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/FONT-WOFF", null, null);		// Test [16] <- Index 16
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0017 = "cdn1.onlineaccess1.com";
		int    requestPort0017 = 443;
		String requestFile0017 = "/cdn/3249/4.1.1.4/000038/assets/fonts/OpenSans/OpenSans-Semibold.woff";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0017 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0017);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/FONT-WOFF", header text fragment = [verification disabled], recorded content size = 24952
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/FONT-WOFF", null, null);		// Test [17] <- Index 17
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0018 = "cdn1.onlineaccess1.com";
		int    requestPort0018 = 443;
		String requestFile0018 = "/cdn/3249/4.1.1.4/000038/assets/fonts/fontello.woff" +
				"?29134652";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0018 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0018);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/FONT-WOFF", header text fragment = [verification disabled], recorded content size = 36116
		// content test algorithm: none - no content check
		...
		String requestHost0019 = "cdn1.onlineaccess1.com";
		int    requestPort0019 = 443;
		String requestFile0019 = "/cdn/3249/4.1.1.4/000038/assets/images/desktop-background.jpg";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0019 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0019);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 963353
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, null);		// Test [19] <- Index 19
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0020 = "cdn1.onlineaccess1.com";
		int    requestPort0020 = 443;
		String requestFile0020 = "/cdn/3249/4.1.1.4/000038/assets/overlays/overlays_desktop.js";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0020 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0020);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
	...
	boolean executePage_1_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0021 = "demo.q2ebanking.com";
		int    requestPort0021 = 443;
		String requestFile0021 = "/usameribankonline_test_2_927002/mobilews/marketingMessageUnauth" +
				"?ws1230";
		String requestContent0021 = 
				"{\"marketingName\":\"Tablet_Interstitial\",\"userId\":\"-q2user\"}";
		requestContent0021 = Lib.replaceTextPattern(requestContent0021, "-q2user", userId, 0);		// search and replace content fragment "-q2user" with variable 'userId' on all occurrences
		...
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0021);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		if (debugLoops)
		{
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [21] <- Index 21
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0022 = "demo.q2ebanking.com";
		int    requestPort0022 = 443;
		String requestFile0022 = "/usameribankonline_test_2_927002/mobilews/marketingMessageUnauth" +
				"?ws1233";
		String requestContent0022 = 
				"{\"marketingName\":\"Tablet_Interstitial\",\"userId\":\"-q2user\"}";
		requestContent0022 = Lib.replaceTextPattern(requestContent0022, "-q2user", userId, 0);		// search and replace content fragment "-q2user" with variable 'userId' on all occurrences

		byte[] requestContentBinary0022 = requestContent0022.getBytes("UTF-8");
		String requestHeader0022 = "POST " + requestFile0022 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"Content-Type: application/json; charset=utf-8\r\n" + 
				"Content-Length: " + requestContentBinary0022.length + "\r\n" + 		// Content-Length: 58
				"DNT: 1\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0022, requestHost0022, requestPort0022, requestHeader0022, requestContentBinary0022, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		
		LoadtestPluginContext urlPluginBeforeCallContext0022 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0022);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		if (debugLoops)
		{
		...
		requestContentBinary0022 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());

		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 357
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [22] <- Index 22
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0024 = "demo.q2ebanking.com";
		int    requestPort0024 = 443;
		String requestFile0024 = "/usameribankonline_test_2_927002/mobilews/logonUser" +
				"?ws1262";
		String requestContent0024 = 
				"{\"clientId\":null,\"device\":\"desktop\",\"versions\":{\"EMBER\":\"1.10.0\",\"APP\":\"4.1.1.4\",\"CLIENTFW\":false,\"CONTAINERVER\":false,\"JQUERY\":\"1.11.3\",\"ONLINE\":\"4.1.1.4\"},\"userId\":\"-q2user\",\"password\":\"Active#1!!\"}";
		requestContent0024 = Lib.replaceTextPattern(requestContent0024, "Active#1!!", password, 0);		// search and replace content fragment "Active#1!!" with variable 'password' on all occurrences
		requestContent0024 = Lib.replaceTextPattern(requestContent0024, "-q2user", userId, 0);		// search and replace content fragment "-q2user" with variable 'userId' on all occurrences
		...
		LoadtestPluginContext urlPluginBeforeCallContext0024 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0024);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		if (debugLoops)
		{
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 4344
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "" + userName + "");		// Test [24] <- Index 24
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// setup var extractor for 'xcsrf_token'
		ContentTokenExtractorItem contentTokenExtractorItem0024_1 = new ContentTokenExtractorItem("token", "\t\n\r \"(),={}", 2);
		contentTokenExtractor.addTokenExtractorItem(contentTokenExtractorItem0024_1);

		contentTokenExtractor.extractTokens();
		try
		{
			// extract var 'xcsrf_token'
			xcsrf_token = contentTokenExtractorItem0024_1.getResultToken();
			log("<<< xcsrf_token = " + xcsrf_token);
		}
		catch (Exception e) { xcsrf_token = null; log(e); }
		if (xcsrf_token == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'xcsrf_token' from content token pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_3(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0026 = "cdn1.onlineaccess1.com";
		int    requestPort0026 = 443;
		String requestFile0026 = "/cdn/3249/4.1.1.4/000038/assets/images/logos/logo.png";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0026 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0026);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 38778
		// content test algorithm: none - no content check
		...
		String requestHost0027 = "cdn1.onlineaccess1.com";
		int    requestPort0027 = 443;
		String requestFile0027 = "/cdn/3249/4.1.1.4/000038/assets/images/background.png";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0027 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0027);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 8353
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, null);		// Test [27] <- Index 27
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0028 = "cdn1.onlineaccess1.com";
		int    requestPort0028 = 443;
		String requestFile0028 = "/cdn/3249/4.1.1.4/000038/assets/images/fullscreeninter.jpg";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0028 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0028);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 141176
		// content test algorithm: none - no content check
		...
		String requestHost0029 = "demo.q2ebanking.com";
		int    requestPort0029 = 443;
		String requestFile0029 = "/usameribankonline_test_2_927002/mobilews/uuxConfiguration";
		String requestHeader0029 = "GET " + requestFile0029 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-US,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"x-csrf: s7Hp7SNHLUqqC0cYhBavgQ2\r\n" + 
				"X-Requested-With: XMLHttpRequest\r\n" + 
				"DNT: 1\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"Content-Length: 0\r\n" + 
		...
		testURL.replaceRequestHeaderFieldTextPattern("s7Hp7SNHLUqqC0cYhBavgQ2", xcsrf_token);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		
		LoadtestPluginContext urlPluginBeforeCallContext0029 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0029);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 159
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "endUserMessage");		// Test [29] <- Index 29
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0030 = "demo.q2ebanking.com";
		int    requestPort0030 = 443;
		String requestFile0030 = "/usameribankonline_test_2_927002/mobilews/nav";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0030 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0030);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 18988
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "\"availableForLanding\":true");		// Test [30] <- Index 30
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0031 = "demo.q2ebanking.com";
		int    requestPort0031 = 443;
		String requestFile0031 = "/usameribankonline_test_2_927002/mobilews/policySet/user";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0031 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0031);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 32355
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "" + userName + "");		// Test [31] <- Index 31
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0032 = "demo.q2ebanking.com";
		int    requestPort0032 = 443;
		String requestFile0032 = "/usameribankonline_test_2_927002/mobilews/userProfile";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0032 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0032);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 466
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "firstName", "AND", "lastName");		// Test [32] <- Index 32
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0033 = "demo.q2ebanking.com";
		int    requestPort0033 = 443;
		String requestFile0033 = "/usameribankonline_test_2_927002/mobilews/account";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0033 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0033);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 2113
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "\"accountNumber\":", "AND", "\"balanceToDisplay\":");		// Test [33] <- Index 33
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0034 = "demo.q2ebanking.com";
		int    requestPort0034 = 443;
		String requestFile0034 = "/usameribankonline_test_2_927002/mobilews/audit";
		String requestContent0034 = 
				"{\"auditShortName\":\"NavigationEvent\",\"auditDetails\":\"landingPage\"}";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0034 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0034);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		if (debugLoops)
		{
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 121
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "\"endUserMessage\":null", "AND", "\"exceptionMessage\":null");		// Test [34] <- Index 34
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0035 = "demo.q2ebanking.com";
		int    requestPort0035 = 443;
		String requestFile0035 = "/usameribankonline_test_2_927002/mobilews/marketingMessage/dashboard.top.banner";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0035 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0035);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 68594
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [35] <- Index 35
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0036 = "demo.q2ebanking.com";
		int    requestPort0036 = 443;
		String requestFile0036 = "/usameribankonline_test_2_927002/mobilews/marketingMessage/dashboard.right.small";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0036 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0036);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 357
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [36] <- Index 36
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0037 = "demo.q2ebanking.com";
		int    requestPort0037 = 443;
		String requestFile0037 = "/usameribankonline_test_2_927002/mobilews/rdc";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0037 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0037);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 173
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [37] <- Index 37
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0038 = "demo.q2ebanking.com";
		int    requestPort0038 = 443;
		String requestFile0038 = "/usameribankonline_test_2_927002/mobilews/transactions/pending";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0038 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0038);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [38] <- Index 38
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0039 = "demo.q2ebanking.com";
		int    requestPort0039 = 443;
		String requestFile0039 = "/usameribankonline_test_2_927002/mobilews/marketingMessage/dashboard.right.medium";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0039 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0039);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 357
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [39] <- Index 39
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0040 = "demo.q2ebanking.com";
		int    requestPort0040 = 443;
		String requestFile0040 = "/usameribankonline_test_2_927002/mobilews/marketingMessage/banner";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0040 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0040);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 357
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [40] <- Index 40
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0041 = "demo.q2ebanking.com";
		int    requestPort0041 = 443;
		String requestFile0041 = "/usameribankonline_test_2_927002/mobilews/marketingMessage/tablet_notification";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0041 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0041);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 357
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "auditId");		// Test [41] <- Index 41
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0042 = "cdn1.onlineaccess1.com";
		int    requestPort0042 = 443;
		String requestFile0042 = "/cdn/3249/4.1.1.4/000038/assets/overlays/overlays_desktop.js";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0042 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0042);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		String requestHost0043 = "demo.q2ebanking.com";
		int    requestPort0043 = 443;
		String requestFile0043 = "/usameribankonline_test_2_927002/mobilews/marketingMessage/122" +
				"?shortName=dashboard.top.banner";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0043 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		
		// execute plug-in #1 before url call
		log(">>> execute plug-in #1: AppDynamicsIntegrationV4");
		((GenericPluginInterface) plugin0001).setInputParameter(0, vTriggerSnapshot);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginBeforeCallContext0043);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 125936
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "\"shortName\":\"dashboard.top.banner\"");		// Test [43] <- Index 43
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		if ((ParseArgs.hasOption(args, "-h")) || (ParseArgs.hasOption(args, "-help")))
		{
			...
			System.out.println();
			System.out.println("-vTriggerSnapshot <user-input-field>     ->> required argument: Trigger Snapshot");
			System.out.println();
		...
		// parse -vTriggerSnapshot argument or ask --> <user-input-field>: proposed value = "false"
		try
		{
			vTriggerSnapshot = UserInputField.readFromObjectString(USER_INPUT_FIELD_vTriggerSnapshot).readUserInputValue(args);
			if (vTriggerSnapshot.startsWith("\"") && vTriggerSnapshot.endsWith("\""))
				vTriggerSnapshot = vTriggerSnapshot.substring(1, vTriggerSnapshot.length() - 1);
		}
		catch (IOException ie)
		{
			System.out.println("*** INTERNAL ERROR: invalid user input field object-string declaration for 'vTriggerSnapshot' ***");
			ie.printStackTrace();
			System.exit(-2);
		}
		...
		String genericFileName = PerformanceData.proposeFileName("Q2E_USAmeriBank_Login__S", concurrentUsers);
		...
		try
		{
			accounts_txt = new VarInputFileReader("accounts.txt", "//", ";", AbstractInputFileReader.EOF_REOPEN_FILE, prxCharEncoding);
			accounts_txt.setTrimValues(true);
		}
		catch (IOException ie)
		{
		...
		System.out.println("# input file = accounts.txt, file scope = new line per loop");
		if (debugLoops || debugFailedLoops)
		{
			System.out.println("# global var vTriggerSnapshot = " + vTriggerSnapshot);
		}
		System.out.println();
		...
		performanceData.setInfoText("Q2E_USAmeriBank_Login__S");
		...
		performanceData.addUserInputField("Trigger Snapshot", vTriggerSnapshot);		// add user input field name and value to performance data
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				Q2E_USAmeriBank_Login__S simulatedUser = new Q2E_USAmeriBank_Login__S(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
