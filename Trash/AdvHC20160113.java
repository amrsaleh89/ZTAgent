...
// -----------------------------------------
//
// Source: AdvHC20160113.java
// Date  : 08 Mar 2017 09:31:54 PST
 ...
 * Automatically generated load test program.
 */
public class AdvHC20160113 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.10; rv:40.0) Gecko/20100101 Firefox/40.0";
	...
	private static final String EXTERNAL_PARAM_FILE = "AdvHC20160113ExtParams.dat";         // input file name for very large parameter values
	...
	private String lt = null;                            // var declaration from web admin var handler: scope = per loop
	private String SAMLResponse = null;                  // var declaration from web admin var handler: scope = per loop
	private String ticket_2 = null;                      // var declaration from web admin var handler: scope = per loop
	private String challenge = null;                     // var declaration from web admin var handler: scope = per loop
	private String ticket_1 = null;                      // var declaration from web admin var handler: scope = per loop
	private String flash_digest = null;                  // var declaration from web admin var handler: scope = per loop
	private String cpi = null;                           // var declaration from web admin var handler: scope = per loop
	private String SAMLRequest = null;                   // var declaration from web admin var handler: scope = per loop
	

	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public AdvHC20160113()
	{
		super();
	}
	 ...
	 */
	public AdvHC20160113(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log();
		log("<<< lt = " + lt);
		log("<<< SAMLResponse = " + SAMLResponse);
		log("<<< ticket_2 = " + ticket_2);
		log("<<< challenge = " + challenge);
		log("<<< ticket_1 = " + ticket_1);
		log("<<< flash_digest = " + flash_digest);
		log("<<< cpi = " + cpi);
		log("<<< SAMLRequest = " + SAMLRequest);
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		log();
		log("# title: Rakuten LinkShare");
		String requestProt0001 = "https";
		String requestHost0001 = "login.linkshare.com";
		int    requestPort0001 = 443;
		String requestFile0001 = "/sso/login" +
				"?service=http%3A%2F%2Fcli.linksynergy.com%2Fcli%2Fcommon%2Flogin.php";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: login.linkshare.com\r\n" + 
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 1704
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "Rakuten Marketing LLC. All rights reserved.");		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		{
			// extract var 'lt'
			lt = htmlContentParser.getFormTable().getForm(0).getFormItemValue("lt");		// recorded value = LT-429708-ueFjZxd4xyEdRWaSXdtZiKkbEoPyhw
			log("<<< lt = " + lt);
		}
		catch (Exception e) { lt = null; log(e); }
		if (lt == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'lt' from html form parameter";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0002 = "login.linkshare.com";
		int    requestPort0002 = 443;
		String requestFile0002 = "/sso/login" +
				"?service=http%3A%2F%2Fcli.linksynergy.com%2Fcli%2Fcommon%2Flogin.php";
		String requestContent0002 = 
				"lt=" + lt +		// recorded value = LT-429708-ueFjZxd4xyEdRWaSXdtZiKkbEoPyhw
				"&execution=e1s1" +
				"&_eventId=submit" +
		...
		// verify response: status code = 302 or 200, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302,200}, null, null, null);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		try
		{
			// extract var 'ticket_1'
			ticket_1 = testURL.getRedirectLocation().getParameter("ticket");		// recorded value = ST-123787-xPca3s0DEDCrZ9Rs3dqu-sso-01.linkshare.com
			log("<<< ticket_1 = " + ticket_1);
		}
		catch (Exception e) { ticket_1 = null; log(e); }
		if (ticket_1 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'ticket_1' from http redirection";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		String requestHost0003 = "cli.linksynergy.com";
		int    requestPort0003 = 80;
		String requestFile0003 = "/cli/common/login.php" +
				"?ticket=" + ticket_1;		// recorded value = ST-123787-xPca3s0DEDCrZ9Rs3dqu-sso-01.linkshare.com
		String requestHeader0003 = "GET " + requestFile0003 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: cli.linksynergy.com\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n" + 
				"Accept-Language: en-US,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
		...
		String requestHost0004 = "cli.linksynergy.com";
		int    requestPort0004 = 80;
		String requestFile0004 = "/cli/common/login.php";
		...
		{
			// extract var 'cpi'
			cpi = testURL.getRedirectLocation().getParameter("cpi");		// recorded value = cdpe45uY9JBuOdljkIFX1aDHFJpTxkOJbqpXzOwfaGBY%2B7%2FkA3smEA%3D%3D
			log("<<< cpi = " + cpi);
		}
		catch (Exception e) { cpi = null; log(e); }
		if (cpi == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'cpi' from http redirection";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		String requestHost0005 = "dashboard.linkshare.com";
		int    requestPort0005 = 443;
		String requestFile0005 = "/Advertiser/Authenticate/Dashboard/legacyDashGateway.php" +
				"?cpi=" + cpi;		// recorded value = cdpe45uY9JBuOdljkIFX1aDHFJpTxkOJbqpXzOwfaGBY%2B7%2FkA3smEA%3D%3D
		String requestHeader0005 = "GET " + requestFile0005 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: dashboard.linkshare.com\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n" + 
				"Accept-Language: en-US,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
		...
		log();
		log("# title: LinkShare - Home");
		String requestProt0006 = "https";
		String requestHost0006 = "dashboard.linkshare.com";
		int    requestPort0006 = 443;
		String requestFile0006 = "/Advertiser/";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 11247
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "date_range_string = date_range_string + prevstart +");		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0007 = "advhelp.rakutenmarketing.com";
		int    requestPort0007 = 443;
		String requestFile0007 = "/hc";
		String requestHeader0007 = "GET " + requestFile0007 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: advhelp.rakutenmarketing.com\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n" + 
				"Accept-Language: en-US,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
		...
		// verify response: status code = 301, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 111
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {301}, null, null, null);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0008 = "advhelp.rakutenmarketing.com";
		int    requestPort0008 = 443;
		String requestFile0008 = "/hc/en-us";
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 188
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302}, null, null, null);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		log();
		log("# title: Rakuten Affiliate Network - Advertiser Help Center");
		String requestProt0009 = "https";
		String requestHost0009 = "advhelp.rakutenmarketing.com";
		int    requestPort0009 = 443;
		String requestFile0009 = "/hc/en-us/restricted" +
				"?return_to=https%3A%2F%2Fadvhelp.rakutenmarketing.com%2Fhc%2Fen-us";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 1396
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "figcaption figure footer header hgroup main mark meter nav output \\");		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0010 = "rakutenlinkshare.zendesk.com";
		int    requestPort0010 = 443;
		String requestFile0010 = "/auth/v2/host.js";
		String requestHeader0010 = "GET " + requestFile0010 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: rakutenlinkshare.zendesk.com\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: */*\r\n" + 
				"Accept-Language: en-US,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
		...
		// verify response: status code = 200, content type = "TEXT/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 3182
		// content test algorithm: check size +/- 5%, defined content size = 3182
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 3182, 5);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
		...
		String requestHost0011 = "p4.zdassets.com";
		int    requestPort0011 = 443;
		String requestFile0011 = "/hc/assets/hc_restricted-b054a562a19b703e4d016a70e592f0fb.js";
		String requestHeader0011 = "GET " + requestFile0011 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: p4.zdassets.com\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: */*\r\n" + 
				"Accept-Language: en-US,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
		...
		// content test algorithm: check size +/- 5%, defined content size = 187
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 187, 5);		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0012 = "rakutenlinkshare.zendesk.com";
		int    requestPort0012 = 443;
		String requestFile0012 = "/auth/v2/login/signin" +
				"?return_to=https%3A%2F%2Fadvhelp.rakutenmarketing.com%2Fhc%2Fen-us" +
				"&theme=hc" +
				"&locale=1" +
				"&brand_id=339425" +
				"&auth_origin=339425%2Ctrue%2Ctrue";
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 269
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302}, null, null, null);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		log();
		log("# title: Zendesk Auth");
		String requestProt0013 = "https";
		String requestHost0013 = "rakutenlinkshare.zendesk.com";
		int    requestPort0013 = 443;
		String requestFile0013 = "/auth/v2/login/sso" +
				"?auth_origin=339425%2Ctrue%2Ctrue" +
				"&brand_id=339425" +
				"&locale=1" +
				"&return_to=https%3A%2F%2Fadvhelp.rakutenmarketing.com%2Fhc%2Fen-us" +
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 1094
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "new App({targetOrigin: targetOrigin});");		// Test [13] <- Index 13
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// setup var extractor for 'SAMLRequest'
		TextLineTokenExtractorItem lineTokenExtractorItem0013_1 = new TextLineTokenExtractorItem(TextLineTokenExtractorItem.LINE_CONTAINS_PATTERN, "https://login.linkshare.com/idp/profile/SAML2/Redirect/SSO?SAMLRequest", 0, "=&", 6);
		textLineTokenExtractor.addTokenExtractorItem(lineTokenExtractorItem0013_1);

		textLineTokenExtractor.extractTokens();
		try
		{
			// extract var 'SAMLRequest'
			SAMLRequest = lineTokenExtractorItem0013_1.getResultToken();
			log("<<< SAMLRequest = " + SAMLRequest);
		}
		catch (Exception e) { SAMLRequest = null; log(e); }
		if (SAMLRequest == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'SAMLRequest' from text line pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		String requestHost0014 = "login.linkshare.com";
		int    requestPort0014 = 443;
		String requestFile0014 = "/idp/profile/SAML2/Redirect/SSO" +
				"?SAMLRequest=fZFLb8IwEITv%2FIrIdydxQlBkkaAIVAmJVhV9HHpz7K2wcGzqdejj1zekRaKH%0Act2d0cx%2BO198dCY6gkftbEVYnJJFPZmj6MyBN33Y2S289YAhGnQW%2BbioSO8t%0AdwI1cis6QB4kf2huNzyLU37wLjjpDLmwXHcIRPBhKECi9aoiJ4OneaZEkbcl%0AnSmVU8agoG2bAm1LKXIxY23ZTgc9Yg9ri0HYUJEsZQVlKU3LR1bwacbz2QuJ%0Ans%2FXDVkkas5hS2ex78A%2FgD9qCU%2FbTUV2IRyQJ4kX%2Bz6ANdrucSc8xF9gFeA%2B%0Alq5LhJSAmJxqknpExccavr5mmyeXyl%2FCdwOL9ereGS0%2Fo8YY9770IAJUJPge%0ASHTjfCfC%2F%2FRYzMaJVvR1lHLohDaNUn6oSJL6J%2FXvK%2BvJNw%3D%3D%0A" +
				"&RelayState=https%3A%2F%2Fadvhelp.rakutenmarketing.com%2Fhc%2Fen-us";
		requestFile0014 = Lib.replaceTextPattern(requestFile0014, "fZFLb8IwEITv%2FIrIdydxQlBkkaAIVAmJVhV9HHpz7K2wcGzqdejj1zekRaKH%0Act2d0cx%2BO198dCY6gkftbEVYnJJFPZmj6MyBN33Y2S289YAhGnQW%2BbioSO8t%0AdwI1cis6QB4kf2huNzyLU37wLjjpDLmwXHcIRPBhKECi9aoiJ4OneaZEkbcl%0AnSmVU8agoG2bAm1LKXIxY23ZTgc9Yg9ri0HYUJEsZQVlKU3LR1bwacbz2QuJ%0Ans%2FXDVkkas5hS2ex78A%2FgD9qCU%2FbTUV2IRyQJ4kX%2Bz6ANdrucSc8xF9gFeA%2B%0Alq5LhJSAmJxqknpExccavr5mmyeXyl%2FCdwOL9ereGS0%2Fo8YY9770IAJUJPge%0ASHTjfCfC%2F%2FRYzMaJVvR1lHLohDaNUn6oSJL6J%2FXvK%2BvJNw%3D%3D%0A", SAMLRequest, 0);		// search and replace request file fragment "fZFLb8IwEITv%2FIrIdydxQlBkkaAIVAmJVhV9HHpz7K2wcGzqdejj1zekRaKH%0Act2d0cx%2BO198dCY6gkftbEVYnJJFPZmj6MyBN33Y2S289YAhGnQW%2BbioSO8t%0AdwI1cis6QB4kf2huNzyLU37wLjjpDLmwXHcIRPBhKECi9aoiJ4OneaZEkbcl%0AnSmVU8agoG2bAm1LKXIxY23ZTgc9Yg9ri0HYUJEsZQVlKU3LR1bwacbz2QuJ%0Ans%2FXDVkkas5hS2ex78A%2FgD9qCU%2FbTUV2IRyQJ4kX%2Bz6ANdrucSc8xF9gFeA%2B%0Alq5LhJSAmJxqknpExccavr5mmyeXyl%2FCdwOL9ereGS0%2Fo8YY9770IAJUJPge%0ASHTjfCfC%2F%2FRYzMaJVvR1lHLohDaNUn6oSJL6J%2FXvK%2BvJNw%3D%3D%0A" with variable 'SAMLRequest' on all occurrences
		...
		String requestHost0015 = "login.linkshare.com";
		int    requestPort0015 = 443;
		String requestFile0015 = "/idp/profile/SAML2/Redirect/SSO" +
				"?execution=e1s1";
		...
		String requestHost0016 = "login.linkshare.com";
		int    requestPort0016 = 443;
		String requestFile0016 = "/idp/Authn/ExtCas" +
				"?conversation=e1s1";
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302}, null, null, null);		// Test [16] <- Index 16
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0017 = "login.linkshare.com";
		int    requestPort0017 = 443;
		String requestFile0017 = "/sso/login" +
				"?service=https%3A%2F%2Flogin.linkshare.com%2Fidp%2FAuthn%2FExtCas%3Fconversation%3De1s1" +
				"&entityId=rakutenlinkshare.zendesk.com";
		...
		try
		{
			// extract var 'ticket_2'
			ticket_2 = testURL.getRedirectLocation().getParameter("ticket");		// recorded value = ST-123798-hEaoOrnaDxyHD6DdmlQs-sso-01.linkshare.com
			log("<<< ticket_2 = " + ticket_2);
		}
		catch (Exception e) { ticket_2 = null; log(e); }
		if (ticket_2 == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'ticket_2' from http redirection";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		String requestHost0018 = "login.linkshare.com";
		int    requestPort0018 = 443;
		String requestFile0018 = "/idp/Authn/ExtCas" +
				"?conversation=e1s1" +
				"&ticket=" + ticket_2;		// recorded value = ST-123798-hEaoOrnaDxyHD6DdmlQs-sso-01.linkshare.com
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {302}, null, null, null);		// Test [18] <- Index 18
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0019 = "login.linkshare.com";
		int    requestPort0019 = 443;
		String requestFile0019 = "/idp/profile/SAML2/Redirect/SSO" +
				"?execution=e1s1" +
				"&_eventId_proceed=1";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 4136
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "you must press the Continue button once to proceed.");		// Test [19] <- Index 19
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		try
		{
			// extract var 'SAMLResponse'
			SAMLResponse = htmlContentParser.getFormTable().getForm(0).getFormItemValue("SAMLResponse");		// recorded value = 
			log("<<< SAMLResponse = " + SAMLResponse);
		}
		catch (Exception e) { SAMLResponse = null; log(e); }
		if (SAMLResponse == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'SAMLResponse' from html form parameter";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		String requestHost0020 = "rakutenlinkshare.zendesk.com";
		int    requestPort0020 = 443;
		String requestFile0020 = "/access/saml";
		String requestContent0020 = 
				"RelayState=https%3A%2F%2Fadvhelp.rakutenmarketing.com%2Fhc%2Fen-us" +
				"&SAMLResponse=" + java.net.URLEncoder.encode(SAMLResponse);		// recorded value = %2BCjxkczpUcmFuc2Zvcm0gQWxnb3JpdGhtPSJodHRwOi8vd3d3LnczLm9yZy8yMDAxLzEwL3htbC1leGMtYzE0biMiLz4KPC9kczpUcmFuc2Zvcm1zPgo8ZHM6RGlnZXN0TWV0aG9kIEFsZ29yaXRobT0iaHR0cDovL3d3dy53My5vcmcvMjAwMS8wNC94bWxlbmMjc2hhMjU2Ii8%2BCjxkczpEaWdlc3RWYWx1ZT5qdlhmcG01MlAyNSswZVNWRENsWW9CdXdPaTNSOGZWQ0VzVU9hVndkeGFnPTwvZHM6RGlnZXN0VmFsdWU%2BCjwvZHM6UmVmZXJlbmNlPgo8L2RzOlNpZ25lZEluZm8%%%2BPC9kczpYNTA5RGF0YT48L2RzOktleUluZm8%2BPC9kczpTaWduYXR1cmU%2BPHNhbWwycDpTdGF0dXM%2BPHNhbWwycDpTdGF0dXNDb2RlIFZhbHVlPSJ1cm46b2FzaXM6bmFtZXM6dGM6U0FNTDoyLjA6c3RhdHVzOlN1Y2Nlc3MiLz48L3NhbWwycDpTdGF0dXM%2BPHNhbWwyOkFzc2VydGlvbiBJRD0iXzhmZjQ2NTc5OTJkOGRjMWNkNzg3OGU0OWMzMzdhMzdiIiBJc3N1ZUluc3RhbnQ9IjIwMTUtMTAtMDhUMTU6NDI6MzkuNDY3WiIgVmVyc2lvbj0iMi4wIiB4bWxuczpzYW1sMj0idXJuOm9hc2lzOm5hbWVzOnRjOlNBTUw6Mi4wOmFzc2VydGlvbiI%%2Bc2F0ZXN0QGV4YW1wbGUuY29tPC9zYW1sMjpOYW1lSUQ%%2BPHNhbWwyOkNvbmRpdGlvbnMgTm90QmVmb3JlPSIyMDE1LTEwLTA4VDE1OjQyOjM5LjQ2N1oiIE5vdE9uT3JBZnRlcj0iMjAxNS0xMC0wOFQxNTo0NzozOS40NjdaIj48c2FtbDI6QXVkaWVuY2VSZXN0cmljdGlvbj48c2FtbDI6QXVkaWVuY2U%2BcmFrdXRlbmxpbmtzaGFyZS56ZW5kZXNrLmNvbTwvc2FtbDI6QXVkaWVuY2U%%2BPC9zYW1sMjpBdXRobkNvbnRleHQ%%2BPC9zYW1sMjpBdHRyaWJ1dGU%2BPHNhbWwyOkF0dHJpYnV0ZSBOYW1lPSJvcmdhbml6YXRpb24iIE5hbWVGb3JtYXQ9InVybjpvYXNpczpuYW1lczp0YzpTQU1MOjIuMDphdHRybmFtZS1mb3JtYXQ6dXJpIj48c2FtbDI6QXR0cmlidXRlVmFsdWU%2BMjQxNzY8L3NhbWwyOkF0dHJpYnV0ZVZhbHVlPjwvc2FtbDI6QXR0cmlidXRlPjxzYW1sMjpBdHRyaWJ1dGUgTmFtZT0iZW1haWwiIE5hbWVGb3JtYXQ9InVybjpvYXNpczpuYW1lczp0YzpTQU1MOjIuMDphdHRybmFtZS1mb3JtYXQ6dXJpIj48c2FtbDI6QXR0cmlidXRlVmFsdWU%%2BPC9zYW1sMjpBdHRyaWJ1dGU%2BPHNhbWwyOkF0dHJpYnV0ZSBOYW1lPSJodHRwOi8vc2NoZW1hcy54bWxzb2FwLm9yZy93cy8yMDA1LzA1L2lkZW50aXR5L2NsYWltcy9zdXJuYW1lIiBOYW1lRm9ybWF0PSJ1cm46b2FzaXM6bmFtZXM6dGM6U0FNTDoyLjA6YXR0cm5hbWUtZm9ybWF0OnVyaSI%2BPHNhbWwyOkF0dHJpYnV0ZVZhbHVlPkNvbnRhd2U8L3NhbWwyOkF0dHJpYnV0ZVZhbHVlPjwvc2FtbDI6QXR0cmlidXRlPjwvc2FtbDI6QXR0cmlidXRlU3RhdGVtZW50Pjwvc2FtbDI6QXNzZXJ0aW9uPjwvc2FtbDJwOlJlc3BvbnNlPg%3D%3D
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 294
		// content test algorithm: none - no content check
		...
		try
		{
			// extract var 'challenge'
			challenge = testURL.getRedirectLocation().getParameter("challenge");		// recorded value = BcKAZkHa4uzO5TsuUqOOx0U8F
			log("<<< challenge = " + challenge);
		}
		catch (Exception e) { challenge = null; log(e); }
		if (challenge == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'challenge' from http redirection";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
		...
		try
		{
			// extract var 'flash_digest'
			flash_digest = testURL.getRedirectLocation().getParameter("flash_digest");		// recorded value = 6da1217d48570a8f2d7b84200b4a1111174533a8
			log("<<< flash_digest = " + flash_digest);
		}
		catch (Exception e) { flash_digest = null; log(e); }
		if (flash_digest == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'flash_digest' from http redirection";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
	...
	boolean executePage_1_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0021 = "advhelp.rakutenmarketing.com";
		int    requestPort0021 = 443;
		String requestFile0021 = "/access/return_to" +
				"?challenge=" + challenge +		// recorded value = BcKAZkHa4uzO5TsuUqOOx0U8F
				"&flash_digest=" + flash_digest +		// recorded value = 6da1217d48570a8f2d7b84200b4a1111174533a8
				"&locale=en-US-x-1" +
				"&return_to=https://advhelp.rakutenmarketing.com/hc/en-us";
		...
		String requestHost0022 = "advhelp.rakutenmarketing.com";
		int    requestPort0022 = 443;
		String requestFile0022 = "/hc/en-us" +
				"?flash_digest=" + flash_digest;		// recorded value = 6da1217d48570a8f2d7b84200b4a1111174533a8
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 23878
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "HELP CENTER", "OR", "3 easy ways to find what you need fast");		// Test [22] <- Index 22
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("AdvHC20160113", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 22;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("AdvHC20160113");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				AdvHC20160113 simulatedUser = new AdvHC20160113(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
