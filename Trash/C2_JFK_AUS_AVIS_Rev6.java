...
// -----------------------------------------
//
// Source: C2_JFK_AUS_AVIS_Rev6.java
// Date  : 27 Mar 2017 15:22:47 PST
 ...
 * Automatically generated load test program.
 */
public class C2_JFK_AUS_AVIS_Rev6 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "C2_JFK_AUS_AVIS_Rev6ExtParams.dat";  // input file name for very large parameter values
	...
	private static String destination = "FLL";                                   // var declaration from web admin var handler: scope = global
	...
	private static String origin = "BOS";                                        // var declaration from web admin var handler: scope = global
	 ...
	 */
	public C2_JFK_AUS_AVIS_Rev6(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		ContentTokenExtractorItem contentTokenExtractorItem0006_1 = new ContentTokenExtractorItem("X-", "\t\n\r \"(),={}", -1);
		contentTokenExtractor.addTokenExtractorItem(contentTokenExtractorItem0006_1);

		contentTokenExtractor.extractTokens();
		try
		{
			// extract var 'UniqueState'
			UniqueState = contentTokenExtractorItem0006_1.getResultToken();
			log("<<< UniqueState = " + UniqueState);
		}
		...
		String requestFile0007 = "/shop/search/app/partials/main.html";
		...
		String requestFile0008 = "/assets/globals/partials/footer.html";
		...
		String requestFile0010 = "/includes/bookjetblueSidekick.aspx";
		...
		String requestHost0011 = "www2.jetblue.com";
		int    requestPort0011 = 443;
		String requestFile0011 = "/JetBlueAlerts/MainTitle_Content.aspx";
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		requestContent0013 = Lib.replaceTextPattern(requestContent0013, "AO1ynu1aAQAAFJYlmMXjRAIsjlKLWOcUfbgPaLw0pxGD0ifItjxMH3KDhzuj", UniqueState, 0);		// search and replace content fragment "AO1ynu1aAQAAFJYlmMXjRAIsjlKLWOcUfbgPaLw0pxGD0ifItjxMH3KDhzuj" with variable 'UniqueState' on all occurrences
		requestContent0013 = Lib.replaceTextPattern(requestContent0013, "AUS", destination, 0);		// search and replace content fragment "AUS" with variable 'destination' on all occurrences
		requestContent0013 = Lib.replaceTextPattern(requestContent0013, "JFK", origin, 0);		// search and replace content fragment "JFK" with variable 'origin' on all occurrences
		...
		testURL.replaceRequestHeaderFieldTextPattern("AUS", destination);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.replaceRequestHeaderFieldTextPattern("JFK", origin);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		if (debugLoops)
		...
		ContentTokenExtractorItem contentTokenExtractorItem0013_1 = new ContentTokenExtractorItem("SessionID", "\t\n\r \"(),={}", 2);
		contentTokenExtractor.addTokenExtractorItem(contentTokenExtractorItem0013_1);

		// setup var extractor for 'Version'
		ContentTokenExtractorItem contentTokenExtractorItem0013_2 = new ContentTokenExtractorItem("https://book.jetblue.com/B6/css/all.css?version", "\t\n\r \"(),={}", 1);
		contentTokenExtractor.addTokenExtractorItem(contentTokenExtractorItem0013_2);

		contentTokenExtractor.extractTokens();
		try
		{
			// extract var 'RJ2Session'
			RJ2Session = contentTokenExtractorItem0013_1.getResultToken();
			log("<<< RJ2Session = " + RJ2Session);
		}
		...
		{
			// extract var 'Version'
			Version = contentTokenExtractorItem0013_2.getResultToken();
			log("<<< Version = " + Version);
		}
		...
		requestFile0015 = Lib.replaceTextPattern(requestFile0015, "201702040020", Version, 0);		// search and replace request file fragment "201702040020" with variable 'Version' on all occurrences
		...
		String requestFile0018 = "/B6/html/empty.html" +
				"?version=201702040020";
		requestFile0018 = Lib.replaceTextPattern(requestFile0018, "201702040020", Version, 0);		// search and replace request file fragment "201702040020" with variable 'Version' on all occurrences
	...
	boolean executePage_3(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0025 = "/B6/Merchandizing.do" +
				"?touchPoint=FLIGHT_RESULTS";
		...
		String requestFile0026 = "/travel/api_cross_sell.cfm" +
		...
		String requestHost0027 = "vacations.jetblue.com";
		int    requestPort0027 = 443;
		String requestFile0027 = "/travel/api_cross_sell_ajax_update.cfm" +
				"?nav=inpathdefault";
		String requestContent0027 = 
				"crossSellToken=194775c5-7d56-4956-97c6-6a169149cd16";
		byte[] requestContentBinary0027 = requestContent0027.getBytes("UTF-8");
		String requestHeader0027 = "POST " + requestFile0027 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Content-Length: " + requestContentBinary0027.length + "\r\n" + 		// Content-Length: 51
				"Connection: Keep-Alive\r\n" + 
				"\r\n";

		// execute request
		testURL = new HttpTestURL(requestProt0027, requestHost0027, requestPort0027, requestHeader0027, requestContentBinary0027, requestTimeout, socketPool, cookieHandler, httpLogVectorObject);
		testURL.replaceRequestHeaderFieldTextPattern("AUS", destination);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.replaceRequestHeaderFieldTextPattern("JFK", origin);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.setDNSContext(userDNSContext);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		...
		requestContentBinary0027 = null;		// support garbage collector to reduce memory
		log("   " + testURL.getShortResultText());
		...
		String requestFile0028 = "/travel/api_cross_sell/record_cross_sell_load_event.cfm" +
				"?crossSellToken=194775c5-7d56-4956-97c6-6a169149cd16" +
				"&nav=inpathdefault";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "flowValidationResult: \"OK\"");		// Test [29] <- Index 29
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0030 = "/B6/ValidateFormAction.do";
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "status: \"success\",");		// Test [30] <- Index 30
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0031 = "/B6/en/pages/tdp/interstitials/hotelSelection.html" +
				"?version=201702040020";
		requestFile0031 = Lib.replaceTextPattern(requestFile0031, "201702040020", Version, 0);		// search and replace request file fragment "201702040020" with variable 'Version' on all occurrences
		...
		String requestFile0032 = "/B6/AirVerifyFareFamiliesItineraryCheckLogin.do";
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "status: \"success\",");		// Test [32] <- Index 32
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0033 = "/B6/TravelersDetailsForwardAction.do";
		...
		String requestFile0035 = "/B6/MerchandizingConfig.do" +
		...
		String requestFile0036 = "/B6/Merchandizing.do" +
				"?touchPoint=PASSENGER_DETAILS";
	...
	boolean executePage_4(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0038 = "/B6/ValidateFormAction.do";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 327
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "status: \"success\",");		// Test [38] <- Index 38
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0039 = "www.jetblue.com";
		int    requestPort0039 = 443;
		String requestFile0039 = "/content/snippets/interstitials/booking/index3.html" +
				"?version=201702040020";
		requestFile0039 = Lib.replaceTextPattern(requestFile0039, "201702040020", Version, 0);		// search and replace request file fragment "201702040020" with variable 'Version' on all occurrences
		...
		String requestFile0040 = "/B6/UpdateReservation.do";
		...
		String requestFile0041 = "/B6/ReservationSeatSelectionForward.do";
		...
		String requestFile0042 = "/B6/ValidateFlow.do";
		...
		String requestFile0043 = "/B6/MerchandizingConfig.do" +
		...
		String requestFile0044 = "/B6/Merchandizing.do" +
				"?touchPoint=SEAT_SELECTION_BOT";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 327
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "status: \"success\",");		// Test [45] <- Index 45
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		requestFile0046 = Lib.replaceTextPattern(requestFile0046, "AUS", destination, 0);		// search and replace request file fragment "AUS" with variable 'destination' on all occurrences
		requestFile0046 = Lib.replaceTextPattern(requestFile0046, "JFK", origin, 0);		// search and replace request file fragment "JFK" with variable 'origin' on all occurrences
	...
	boolean executePage_5(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0048 = "/B6/ReservationSelectSeat.do";
		...
		String requestFile0050 = "/B6/AirRetrieveSeatMapAction.do" +
				...
				"&cacheKey=-2137776623" +
				"&seatMapEditableMode=true";
		requestFile0050 = Lib.replaceTextPattern(requestFile0050, "AUS", destination, 0);		// search and replace request file fragment "AUS" with variable 'destination' on all occurrences
		requestFile0050 = Lib.replaceTextPattern(requestFile0050, "JFK", origin, 0);		// search and replace request file fragment "JFK" with variable 'origin' on all occurrences
	...
	boolean executePage_6(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0052 = "/B6/ReservationSelectSeat.do";
		...
		String requestFile0053 = "/B6/ValidateFormAction.do";
		...
		String requestFile0054 = "/content/snippets/interstitials/booking/index4.html" +
				"?version=201702040020";
		requestFile0054 = Lib.replaceTextPattern(requestFile0054, "201702040020", Version, 0);		// search and replace request file fragment "201702040020" with variable 'Version' on all occurrences
		...
		String requestFile0055 = "/B6/ReservationStoreSeatSelection.do";
		...
		String requestFile0056 = "/B6/MerchandizingSearchShowPageForward.do" +
				"?flowId=seatsFlow";
		...
		String requestFile0058 = "/B6/MerchandizingConfig.do" +
		...
		String requestFile0060 = "/B6/Merchandizing.do" +
				"?touchPoint=BOOKING_PREPAYMENT";
	...
	boolean executePage_7(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0063 = "/B6/ReservationAddMerchandizingOptionsFromSeats.do";
		...
		String requestFile0064 = "/B6/PaymentForward.do";
		...
		String requestFile0065 = "/B6/ValidateFlow.do";
		...
		String requestFile0067 = "/B6/UpdateCreditCard.do";
		...
		String requestFile0068 = "/B6/MerchandizingConfig.do" +
		...
		String requestFile0069 = "/B6/Merchandizing.do" +
				"?touchPoint=PAYMENT_TOP";
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("C2_JFK_AUS_AVIS_Rev6", concurrentUsers);
		...
		if (debugLoops || debugFailedLoops)
		{
			...
			System.out.println("# global var destination = " + destination);
			System.out.println("# global var origin = " + origin);
		}
		System.out.println();
		...
		final int MAX_THREAD_STEPS = 63;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("C2_JFK_AUS_AVIS_Rev6");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				C2_JFK_AUS_AVIS_Rev6 simulatedUser = new C2_JFK_AUS_AVIS_Rev6(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
