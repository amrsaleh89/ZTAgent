...
// -----------------------------------------
//
// Source: subcatbrow.java
// Date  : 25 Jan 2017 20:47:19 ECT
 ...
 * Automatically generated load test program.
 */
public class subcatbrow extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Windows NT 6.3; WOW64; rv:51.0) Gecko/20100101 Firefox/51.0";
	...
	private static final boolean CONTAINS_EXTERNAL_RESOURCES = true;       // note: external resources are typically additional Java library files (*.jar files) invoked by self-developed plug-ins. Consider that Input Files and the Main Class of Plug-Ins are NOT external resources in this context because ZebraTester knows already their declarations.
	...
	private static final String EXTERNAL_PARAM_FILE = "subcatbrowExtParams.dat";            // input file name for very large parameter values
	...
	private static VarInputFileReader Data_csv = null;               // input file Data.csv, file scope: new line per user
	...
	private String SubCat = null;                                         // var declaration from web admin var handler: scope = per user
	 ...
	 */
	public subcatbrow(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
		...
		try
		{
			...
			{
				if (!Data_csv.getNextLine())
					abortByEofInputFile();
				// extract vars from input file: scope = per user
				SubCat = new String(Data_csv.getLineVar(1));
			}
		}
		catch (Exception ie)
		{
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log();
		log();
		log("# Page #2: Home page");
		log("# ------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: Home page", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
		...
		log();
		log();
		log("# Page #3: sub cate page");
		log("# ----------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #3: sub cate page", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 33499
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "Our delivery team will place furniture in the rooms of your choice");		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [3] <- WEB ADMIN Index 18 ---
		String requestProt0003 = "http";
		String requestHost0003 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0003 = 80;
		String requestFile0003 = "/Content/js/deferralHelpers.js";
		...
		// content test algorithm: check size +/- 5%, defined content size = 1954
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 1954, 5);		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
		...
		String requestHost0004 = "maxcdn.bootstrapcdn.com";
		int    requestPort0004 = 80;
		String requestFile0004 = "/font-awesome/4.3.0/css/font-awesome.min.css";
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 5969
		// content test algorithm: check size +/- 5%, defined content size = 5969
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 5969, 5);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0005 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0005 = 80;
		String requestFile0005 = "/bundles/mainLayoutScripts.js" +
				"?v=K1dpGRauy3lRo_0B1NxAJsY1ZgeWCe6FVm8d9GtP7JQ1";
		...
		// verify response: status code = 200, content type = "TEXT/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 217699
		// content test algorithm: check size +/- 5%, defined content size = 217699
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 217699, 5);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [6] <- WEB ADMIN Index 21 ---
		String requestProt0006 = "http";
		String requestHost0006 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0006 = 80;
		String requestFile0006 = "/Content/js/deferralSupport.js";
		...
		// verify response: status code = 200, content type = "APPLICATION/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 665
		// content test algorithm: check size +/- 5%, defined content size = 665
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 665, 5);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [7] <- WEB ADMIN Index 22 ---
		String requestProt0007 = "http";
		String requestHost0007 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0007 = 80;
		String requestFile0007 = "/Content/css/main-webforms.css";
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 8402
		// content test algorithm: check size +/- 5%, defined content size = 8402
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 8402, 5);		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [8] <- WEB ADMIN Index 23 ---
		String requestProt0008 = "http";
		String requestHost0008 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0008 = 80;
		String requestFile0008 = "/bundles/scripts-head.js" +
				"?v=xgXFSTR3cnfLnex33z_obGrO5HDIAHVjAMHKYgJb_zw1";
		...
		// verify response: status code = 200, content type = "TEXT/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 70299
		// content test algorithm: check size +/- 5%, defined content size = 70299
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 70299, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [9] <- WEB ADMIN Index 25 ---
		String requestProt0009 = "http";
		String requestHost0009 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0009 = 80;
		String requestFile0009 = "/css/external/external.css" +
				"?v=S-zQ78JrAzML_1efeX5_PcvcUJkva-SJIe1tYmUSk5U1";
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 5972
		// content test algorithm: check size +/- 5%, defined content size = 5972
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 5972, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0010 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0010 = 80;
		String requestFile0010 = "/css/bundle.css" +
				"?v=a6KLBZCAUFzG2nBJNVcgDCe1yJz-fNMA8_0VR1z2sGo1";
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 118772
		// content test algorithm: check size +/- 5%, defined content size = 118772
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/CSS", null, 118772, 5);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0011 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0011 = 80;
		String requestFile0011 = "/Content/js/main-webforms.js";
		...
		// verify response: status code = 200, content type = "APPLICATION/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 135192
		...
		// --- HTTP REQUEST: Test [12] <- WEB ADMIN Index 34 ---
		String requestProt0012 = "http";
		String requestHost0012 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0012 = 80;
		String requestFile0012 = "/css/fonts/ProximaNova/proximanova-regular_0-webfont.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-FONT-WOFF", header text fragment = [verification disabled], recorded content size = 26636
		// content test algorithm: check size +/- 5%, defined content size = 26636
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-FONT-WOFF", null, 26636, 5);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [13] <- WEB ADMIN Index 39 ---
		String requestProt0013 = "http";
		String requestHost0013 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0013 = 80;
		String requestFile0013 = "/images/hamburger.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 631
		// content test algorithm: check size +/- 5%, defined content size = 631
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 631, 5);		// Test [13] <- Index 13
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [14] <- WEB ADMIN Index 40 ---
		String requestProt0014 = "http";
		String requestHost0014 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0014 = 80;
		String requestFile0014 = "/images/cart-icon.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 1215
		// content test algorithm: check size +/- 5%, defined content size = 1215
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 1215, 5);		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [15] <- WEB ADMIN Index 41 ---
		String requestProt0015 = "http";
		String requestHost0015 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0015 = 80;
		String requestFile0015 = "/images/location-icon.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 1203
		// content test algorithm: check size +/- 5%, defined content size = 1203
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 1203, 5);		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [16] <- WEB ADMIN Index 42 ---
		String requestProt0016 = "http";
		String requestHost0016 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0016 = 80;
		String requestFile0016 = "/images/search-icon2.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 1568
		// content test algorithm: check size +/- 5%, defined content size = 1568
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 1568, 5);		// Test [16] <- Index 16
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [17] <- WEB ADMIN Index 43 ---
		String requestProt0017 = "http";
		String requestHost0017 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0017 = 80;
		String requestFile0017 = "/images/use-icon.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 1163
		// content test algorithm: check size +/- 5%, defined content size = 1163
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 1163, 5);		// Test [17] <- Index 17
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [18] <- WEB ADMIN Index 45 ---
		String requestProt0018 = "http";
		String requestHost0018 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0018 = 80;
		String requestFile0018 = "/images/sprite.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 1072
		// content test algorithm: check size +/- 5%, defined content size = 1072
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 1072, 5);		// Test [18] <- Index 18
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [19] <- WEB ADMIN Index 46 ---
		String requestProt0019 = "http";
		String requestHost0019 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0019 = 80;
		String requestFile0019 = "/css/fonts/ProximaNova/proximanova-light_0-webfont.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-FONT-WOFF", header text fragment = [verification disabled], recorded content size = 26396
		// content test algorithm: check size +/- 5%, defined content size = 26396
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-FONT-WOFF", null, 26396, 5);		// Test [19] <- Index 19
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [20] <- WEB ADMIN Index 47 ---
		String requestProt0020 = "http";
		String requestHost0020 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0020 = 80;
		String requestFile0020 = "/css/fonts/ProximaNova/proximanova-bold_0-webfont.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-FONT-WOFF", header text fragment = [verification disabled], recorded content size = 26840
		// content test algorithm: check size +/- 5%, defined content size = 26840
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-FONT-WOFF", null, 26840, 5);		// Test [20] <- Index 20
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [21] <- WEB ADMIN Index 48 ---
		String requestProt0021 = "http";
		String requestHost0021 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0021 = 80;
		String requestFile0021 = "/css/fonts/ProximaNova/proximanova-semibold_0-webfont.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-FONT-WOFF", header text fragment = [verification disabled], recorded content size = 26728
		// content test algorithm: check size +/- 5%, defined content size = 26728
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-FONT-WOFF", null, 26728, 5);		// Test [21] <- Index 21
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_2_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [22] <- WEB ADMIN Index 49 ---
		String requestProt0022 = "http";
		String requestHost0022 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0022 = 80;
		String requestFile0022 = "/Content/font/font-awesome/fontawesome-webfont.woff2" +
				"?v=4.6.3";
		...
		// verify response: status code = 200, content type = "FONT/WOFF2", header text fragment = [verification disabled], recorded content size = 71896
		// content test algorithm: check size +/- 5%, defined content size = 71896
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "FONT/WOFF2", null, 71896, 5);		// Test [22] <- Index 22
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [23] <- WEB ADMIN Index 50 ---
		String requestProt0023 = "http";
		String requestHost0023 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0023 = 80;
		String requestFile0023 = "/img/carousel/arrow-prev.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 250
		// content test algorithm: check size +/- 5%, defined content size = 250
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 250, 5);		// Test [23] <- Index 23
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [24] <- WEB ADMIN Index 51 ---
		String requestProt0024 = "http";
		String requestHost0024 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0024 = 80;
		String requestFile0024 = "/img/carousel/arrow-next.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 251
		// content test algorithm: check size +/- 5%, defined content size = 251
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 251, 5);		// Test [24] <- Index 24
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [25] <- WEB ADMIN Index 113 ---
		String requestProt0025 = "https";
		String requestHost0025 = "js-agent.newrelic.com";
		int    requestPort0025 = 443;
		String requestFile0025 = "/nr-1016.min.js";
		...
		// verify response: status code = 200, content type = "APPLICATION/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 8830
		// content test algorithm: check size +/- 5%, defined content size = 8830
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, 8830, 5);		// Test [25] <- Index 25
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [26] <- WEB ADMIN Index 137 ---
		String requestProt0026 = "http";
		String requestHost0026 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0026 = 80;
		String requestFile0026 = "/tc-chat-ashleyfurn.html" +
				"?IFRAME";
		...
		// verify response: status code = 404, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 113073
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {404}, null, null, null);		// Test [26] <- Index 26
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [27] <- WEB ADMIN Index 167 ---
		String requestProt0027 = "https";
		String requestHost0027 = "bam.nr-data.net";
		int    requestPort0027 = 443;
		String requestFile0027 = "/1/8434cbb159" +
				"?a=33033972" +
				"&v=1016.8b58850" +
				"&to=ZFxRMksHXEBSUxJaXV0WcjVpSV5SSl8TR0EcVFIPVwpTSlxFEh1TQElL" +
				"&rst=20477" +
				"&ref=http://ashcomm8.cds.ashleyretail.com/" +
				"&ap=27" +
				"&be=1834" +
				"&fe=15172" +
				"&dc=5972" +
				"&perf=%7B%22timing%22:%7B%22of%22:1485209128282,%22n%22:0,%22u%22:961,%22ue%22:981,%22f%22:62,%22dn%22:102,%22dne%22:105,%22c%22:105,%22ce%22:105,%22rq%22:107,%22rp%22:954,%22rpe%22:956,%22dl%22:961,%22di%22:7782,%22ds%22:7804,%22de%22:8502,%22dc%22:17004,%22l%22:17004,%22le%22:17100%7D,%22navigation%22:%7B%7D%7D" +
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 57, 5);		// Test [27] <- Index 27
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [28] <- WEB ADMIN Index 228 ---
		String requestProt0028 = "https";
		String requestHost0028 = "bam.nr-data.net";
		int    requestPort0028 = 443;
		String requestFile0028 = "/1/8434cbb159" +
				"?a=33033972" +
				"&v=1016.8b58850" +
				"&to=ZFxRMksHXEBSUxJaXV0WcjVpSV5SSl8TR0EcVFIPVwpTSlxFEh1TQElL" +
				"&rst=9925" +
				"&ref=http://ashcomm8.cds.ashleyretail.com/tc-chat-ashleyfurn.html" +
				"&ap=32" +
				"&be=2648" +
				"&fe=5956" +
				"&dc=3141" +
				"&perf=%7B%22timing%22:%7B%22of%22:1485209148809,%22n%22:0,%22f%22:0,%22dn%22:0,%22dne%22:0,%22c%22:0,%22ce%22:0,%22rq%22:0,%22rp%22:0,%22rpe%22:0,%22dl%22:1115,%22di%22:5761,%22ds%22:5787,%22de%22:5979,%22dc%22:8602,%22l%22:8603,%22le%22:8667%7D,%22navigation%22:%7B%7D%7D" +
				"&jsonp=NREUM.setToken";
		...
		// verify response: status code = 200, content type = "TEXT/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 57
		// content test algorithm: check size +/- 5%, defined content size = 57
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 57, 5);		// Test [28] <- Index 28
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	{
		// # Page #3: sub cate page
		// # ----------------------



		// --- HTTP REQUEST: Test [30] <- WEB ADMIN Index 244 ---
		String requestProt0030 = "http";
		String requestHost0030 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0030 = 80;
		String requestFile0030 = "/c/furniture/living-room/loveseats/" +
				"?sortby=featured" +
				"&sortby=featured" +
				"&icid=meganav-furniture-menu-loveseats";
		requestFile0030 = Lib.replaceTextPattern(requestFile0030, "/c/furniture/living-room/loveseats/?sortby=featured&sortby=featured&icid=meganav-furniture-menu-loveseats", SubCat, 0);		// search and replace request file fragment "/c/furniture/living-room/loveseats/?sortby=featured&sortby=featured&icid=meganav-furniture-menu-loveseats" with variable 'SubCat' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 67328
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "Loveseats | Ashley Furniture HomeStore");		// Test [30] <- Index 30
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [31] <- WEB ADMIN Index 247 ---
		String requestProt0031 = "http";
		String requestHost0031 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0031 = 80;
		String requestFile0031 = "/Content/css/main.css";
		...
		testURL.replaceRequestHeaderFieldTextPattern("/c/furniture/living-room/loveseats/?sortby=featured&sortby=featured&icid=meganav-furniture-menu-loveseats", SubCat);		// search within all request header fields a text pattern and replace it with a variable value
		testURL.setDNSContext(userDNSContext);
		testURL.setFailureActionType(HttpTestURL.FAILURE_ACTION_IGNORE_ERROR);
		performanceData.setInfoText(threadStep, testURL, -1);		// hint: param #3 is the maximum acceptable response time in milliseconds (-1 = not configured)
		log("[" + threadStep + "] " + testURL.getRequestInfoText() + " ...");
		...
		// verify response: status code = 200, content type = "TEXT/CSS", header text fragment = [verification disabled], recorded content size = 42252
		// content test algorithm: none - no content check
		...
		// --- HTTP REQUEST: Test [32] <- WEB ADMIN Index 248 ---
		String requestProt0032 = "http";
		String requestHost0032 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0032 = 80;
		String requestFile0032 = "/Content/js/jquery-3.1.0.min.js";
		...
		// verify response: status code = 200, content type = "APPLICATION/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 38669
		// content test algorithm: none - no content check
		...
		// --- HTTP REQUEST: Test [33] <- WEB ADMIN Index 249 ---
		String requestProt0033 = "http";
		String requestHost0033 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0033 = 80;
		String requestFile0033 = "/Content/js/main.js";
		...
		// verify response: status code = 200, content type = "APPLICATION/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 271313
		// content test algorithm: none - no content check
		...
		// --- HTTP REQUEST: Test [34] <- WEB ADMIN Index 252 ---
		String requestProt0034 = "http";
		String requestHost0034 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0034 = 80;
		String requestFile0034 = "/Content/icon/hamburger.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 631
		// content test algorithm: check size +/- 5%, defined content size = 631
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 631, 5);		// Test [34] <- Index 34
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [35] <- WEB ADMIN Index 253 ---
		String requestProt0035 = "http";
		String requestHost0035 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0035 = 80;
		String requestFile0035 = "/-/media/ashley/theme/icons/icon-user.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 1163
		// content test algorithm: check size +/- 5%, defined content size = 1163
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 1163, 5);		// Test [35] <- Index 35
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [36] <- WEB ADMIN Index 254 ---
		String requestProt0036 = "http";
		String requestHost0036 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0036 = 80;
		String requestFile0036 = "/-/media/ashley/theme/icons/icon-location.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 1203
		// content test algorithm: check size +/- 5%, defined content size = 1203
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 1203, 5);		// Test [36] <- Index 36
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [37] <- WEB ADMIN Index 255 ---
		String requestProt0037 = "http";
		String requestHost0037 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0037 = 80;
		String requestFile0037 = "/Content/icon/search.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 1568
		// content test algorithm: check size +/- 5%, defined content size = 1568
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 1568, 5);		// Test [37] <- Index 37
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [38] <- WEB ADMIN Index 256 ---
		String requestProt0038 = "http";
		String requestHost0038 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0038 = 80;
		String requestFile0038 = "/-/media/images/header/ahs_logo_horizontal.png" +
				"?h=253" +
				"&w=772";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 12992
		// content test algorithm: check size +/- 5%, defined content size = 12992
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 12992, 5);		// Test [38] <- Index 38
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [39] <- WEB ADMIN Index 257 ---
		String requestProt0039 = "http";
		String requestHost0039 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0039 = 80;
		String requestFile0039 = "/-/media/images/page-categories/living-room/living-room-loveseats-hb-c.jpg" +
				"?la=en";
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 69337
		// content test algorithm: check size +/- 5%, defined content size = 69337
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 69337, 5);		// Test [39] <- Index 39
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [40] <- WEB ADMIN Index 258 ---
		String requestProt0040 = "http";
		String requestHost0040 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0040 = 80;
		String requestFile0040 = "/-/media/ashley/theme/icons/icon-cart.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 1368
		// content test algorithm: check size +/- 5%, defined content size = 1368
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 1368, 5);		// Test [40] <- Index 40
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [41] <- WEB ADMIN Index 259 ---
		String requestProt0041 = "http";
		String requestHost0041 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0041 = 80;
		String requestFile0041 = "/-/media/ashley/theme/icons/smart-buy-icon.jpg";
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 3768
		// content test algorithm: check size +/- 5%, defined content size = 3768
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 3768, 5);		// Test [41] <- Index 41
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [42] <- WEB ADMIN Index 260 ---
		String requestProt0042 = "http";
		String requestHost0042 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0042 = 80;
		String requestFile0042 = "/-/media/images/footer/signupsection/emailenvelopeimage-1.png" +
				"?h=16" +
				"&w=25";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 944
		// content test algorithm: check size +/- 5%, defined content size = 944
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 944, 5);		// Test [42] <- Index 42
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [43] <- WEB ADMIN Index 264 ---
		String requestProt0043 = "http";
		String requestHost0043 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0043 = 80;
		String requestFile0043 = "/apis/v1/cart/count";
		...
		// content test algorithm: check size +/- 5%, defined content size = 21
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 21, 5);		// Test [43] <- Index 43
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [44] <- WEB ADMIN Index 265 ---
		String requestProt0044 = "http";
		String requestHost0044 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0044 = 80;
		String requestFile0044 = "/apis/v1/LeftNavProductAPI" +
				"?currentUrl=http:%2F%2Fashcomm8.cds.ashleyretail.com%2Fc%2Ffurniture%2Fliving-room%2Floveseats%2F%3Fsortby%3Dfeatured%26sortby%3Dfeatured%26icid%3Dmeganav-furniture-menu-loveseats";
		requestFile0044 = Lib.replaceTextPattern(requestFile0044, "%2Fc%2Ffurniture%2Fliving-room%2Floveseats%2F%3Fsortby%3Dfeatured%26sortby%3Dfeatured%26icid%3Dmeganav-furniture-menu-loveseats", java.net.URLEncoder.encode(SubCat), 0);		// search and replace request file fragment "%2Fc%2Ffurniture%2Fliving-room%2Floveseats%2F%3Fsortby%3Dfeatured%26sortby%3Dfeatured%26icid%3Dmeganav-furniture-menu-loveseats" with variable 'SubCat' on all occurrences
		...
		// verify response: status code = 404, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 113247
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {404}, null, null, null);		// Test [44] <- Index 44
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [45] <- WEB ADMIN Index 272 ---
		String requestProt0045 = "http";
		String requestHost0045 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0045 = 80;
		String requestFile0045 = "/Content/font/proxima-nova/proxima-nova-regular-webfont.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-FONT-WOFF", header text fragment = [verification disabled], recorded content size = 26636
		// content test algorithm: check size +/- 5%, defined content size = 26636
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-FONT-WOFF", null, 26636, 5);		// Test [45] <- Index 45
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [46] <- WEB ADMIN Index 273 ---
		String requestProt0046 = "http";
		String requestHost0046 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0046 = 80;
		String requestFile0046 = "/Content/font/proxima-nova/proxima-nova-bold-webfont.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-FONT-WOFF", header text fragment = [verification disabled], recorded content size = 26840
		// content test algorithm: check size +/- 5%, defined content size = 26840
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-FONT-WOFF", null, 26840, 5);		// Test [46] <- Index 46
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [47] <- WEB ADMIN Index 274 ---
		String requestProt0047 = "http";
		String requestHost0047 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0047 = 80;
		String requestFile0047 = "/Content/font/proxima-nova/proxima-nova-semibold-webfont.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-FONT-WOFF", header text fragment = [verification disabled], recorded content size = 26728
		// content test algorithm: check size +/- 5%, defined content size = 26728
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-FONT-WOFF", null, 26728, 5);		// Test [47] <- Index 47
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [48] <- WEB ADMIN Index 275 ---
		String requestProt0048 = "http";
		String requestHost0048 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0048 = 80;
		String requestFile0048 = "/Content/font/proxima-nova/proxima-nova-light-webfont.woff";
		...
		// verify response: status code = 200, content type = "APPLICATION/X-FONT-WOFF", header text fragment = [verification disabled], recorded content size = 26396
		// content test algorithm: check size +/- 5%, defined content size = 26396
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-FONT-WOFF", null, 26396, 5);		// Test [48] <- Index 48
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [49] <- WEB ADMIN Index 320 ---
		String requestProt0049 = "https";
		String requestHost0049 = "bam.nr-data.net";
		int    requestPort0049 = 443;
		String requestFile0049 = "/1/8434cbb159" +
				"?a=33033972" +
				"&v=1016.8b58850" +
				"&to=ZFxRMksHXEBSUxJaXV0WfjB6SWFaR1UFXEBWelwITRRdX19VFBx7XV1WHg%3D%3D" +
				"&rst=11360" +
				"&ref=http://ashcomm8.cds.ashleyretail.com/c/furniture/living-room/loveseats/" +
				"&ap=113" +
				"&be=1269" +
				"&fe=9881" +
				"&dc=7001" +
				"&perf=%7B%22timing%22:%7B%22of%22:1485209174253,%22n%22:0,%22u%22:674,%22ue%22:693,%22f%22:60,%22dn%22:60,%22dne%22:60,%22c%22:60,%22ce%22:60,%22rq%22:91,%22rp%22:668,%22rpe%22:671,%22dl%22:674,%22di%22:8219,%22ds%22:8268,%22de%22:8330,%22dc%22:11148,%22l%22:11149,%22le%22:11182%7D,%22navigation%22:%7B%7D%7D" +
				"&jsonp=NREUM.setToken";
		...
		// verify response: status code = 200, content type = "TEXT/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 57
		// content test algorithm: check size +/- 5%, defined content size = 57
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 57, 5);		// Test [49] <- Index 49
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_3_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [50] <- WEB ADMIN Index 328 ---
		String requestProt0050 = "http";
		String requestHost0050 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0050 = 80;
		String requestFile0050 = "/tc-chat-ashleyfurn.html" +
				"?IFRAME";
		...
		// verify response: status code = 404, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 113073
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {404}, null, null, null);		// Test [50] <- Index 50
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		// --- HTTP REQUEST: Test [51] <- WEB ADMIN Index 340 ---
		String requestProt0051 = "http";
		String requestHost0051 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0051 = 80;
		String requestFile0051 = "/Content/icon/ashley/icon-192x192.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 3148
		// content test algorithm: check size +/- 5%, defined content size = 3148
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 3148, 5);		// Test [51] <- Index 51
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [52] <- WEB ADMIN Index 364 ---
		String requestProt0052 = "http";
		String requestHost0052 = "ashcomm8.cds.ashleyretail.com";
		int    requestPort0052 = 80;
		String requestFile0052 = "/Content/icon/ashley/icon-192x192.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 3148
		// content test algorithm: check size +/- 5%, defined content size = 3148
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 3148, 5);		// Test [52] <- Index 52
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		// --- HTTP REQUEST: Test [53] <- WEB ADMIN Index 399 ---
		String requestProt0053 = "https";
		String requestHost0053 = "bam.nr-data.net";
		int    requestPort0053 = 443;
		String requestFile0053 = "/1/8434cbb159" +
				"?a=33033972" +
				"&v=1016.8b58850" +
				"&to=ZFxRMksHXEBSUxJaXV0WcjVpSV5SSl8TR0EcVFIPVwpTSlxFEh1TQElL" +
				"&rst=12525" +
				...
				"&fe=10324" +
				"&dc=6166" +
				"&perf=%7B%22timing%22:%7B%22of%22:1485209186486,%22n%22:0,%22f%22:0,%22dn%22:0,%22dne%22:0,%22c%22:0,%22ce%22:0,%22rq%22:0,%22rp%22:0,%22rpe%22:0,%22dl%22:613,%22di%22:8098,%22ds%22:8185,%22de%22:8372,%22dc%22:12342,%22l%22:12343,%22le%22:12447%7D,%22navigation%22:%7B%7D%7D" +
				"&jsonp=NREUM.setToken";
		...
		// verify response: status code = 200, content type = "TEXT/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 57
		// content test algorithm: check size +/- 5%, defined content size = 57
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/JAVASCRIPT", null, 57, 5);		// Test [53] <- Index 53
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("subcatbrow", concurrentUsers);
		...
		try
		{
			Data_csv = new VarInputFileReader("Data.csv", "#", ",", AbstractInputFileReader.EOF_REOPEN_FILE, prxCharEncoding);
			Data_csv.setTrimValues(true);
		}
		catch (IOException ie)
		{
		...
		System.out.println("# input file = Data.csv, file scope = new line per user");
		...
		final int MAX_THREAD_STEPS = 51;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("subcatbrow");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				subcatbrow simulatedUser = new subcatbrow(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
