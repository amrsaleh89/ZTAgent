...
public class ww20170314f extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0008 = "/au/checkout/api/plans/csp/1/1/1/0/0";
		...
		ContentTokenExtractorItem contentTokenExtractorItem0008_1 = new ContentTokenExtractorItem(":17611", "\t\n\r \"(),/={}", 14);
		contentTokenExtractor.addTokenExtractorItem(contentTokenExtractorItem0008_1);

		contentTokenExtractor.extractTokens();
		try
		{
			// extract var 'ckval02'
			ckval02 = contentTokenExtractorItem0008_1.getResultToken();
			log("<<< ckval02 = " + ckval02);
		}
	...
	boolean executePage_4(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [12] <- WEB ADMIN Index 62 ---
		String requestProt0012 = "https";
		String requestHost0012 = "api1.weightwatchers.com.au";
		int    requestPort0012 = 443;
		String requestFile0012 = "/signup/v1/member/login/newsLetterOption" +
				"?v=5.0.0-46";
		String requestHeader0012 = "OPTIONS " + requestFile0012 + " HTTP/" + httpProtocolVersion + "\r\n" + 
		...
		// --- HTTP REQUEST: Test [13] <- WEB ADMIN Index 63 ---
		String requestProt0013 = "https";
		String requestHost0013 = "api1.weightwatchers.com.au";
		int    requestPort0013 = 443;
		String requestFile0013 = "/signup/v1/member/login/newsLetterOption" +
				"?v=5.0.0-46";
		...
		ContentTokenExtractorItem contentTokenExtractorItem0013_1 = new ContentTokenExtractorItem("tokenId", "\t\n\r \"(),={}", 2);
		contentTokenExtractor.addTokenExtractorItem(contentTokenExtractorItem0013_1);

		// setup var extractor for 'ckval03'
		ContentTokenExtractorItem contentTokenExtractorItem0013_2 = new ContentTokenExtractorItem("firstName", "\t\n\r \"(),=@{}", 2);
		...
		{
			// extract var 'ckval04'
			ckval04 = contentTokenExtractorItem0013_1.getResultToken();
			log("<<< ckval04 = " + ckval04);
		}
		...
		{
			// extract var 'ckval03'
			ckval03 = contentTokenExtractorItem0013_2.getResultToken();
			log("<<< ckval03 = " + ckval03);
		}
		...
		synchronized (plugin0004)
		{
			...
			((GenericPluginInterface) plugin0004).execute(urlPluginBeforeCallContext0014);			// execute plug-in
		...
		synchronized (plugin0002)
		{
			...
			((GenericPluginInterface) plugin0002).execute(urlPluginBeforeCallContext0014);			// execute plug-in
		...
		synchronized (plugin0003)
		{
			...
			((GenericPluginInterface) plugin0003).execute(urlPluginBeforeCallContext0014);			// execute plug-in
		...
		synchronized (plugin0005)
		{
			...
			((GenericPluginInterface) plugin0005).execute(urlPluginBeforeCallContext0014);			// execute plug-in
		...
		LoadtestPluginContext urlPluginContext0014 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div ng-class=\"hideSpinner\" class=\"spinner-container\"");		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			...
			urlPluginContext0014.setUrlPassed(false);
			...
			((GenericPluginInterface) plugin0001).execute(urlPluginContext0014);			// execute plug-in
		...
		// execute plug-in #1 after url call
		urlPluginContext0014.setUrlPassed(urlCallPassed);
		log(">>> execute plug-in #1: GetCookieValue");
		((GenericPluginInterface) plugin0001).setInputParameter(0, ckname05);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginContext0014);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		if (urlPluginContext0014.getSpecificErrorMessage() != null)
		{
			...
			log(urlPluginContext0014.getSpecificErrorMessage());
			performanceData.getPerformanceDataRecord(threadStep).modifyFailureActionType(HttpTestURL.FAILURE_ACTION_HANDLE_AS_ERROR);
			performanceData.setFailed(threadStep, urlPluginContext0014.getSpecificErrorStatusType(), urlPluginContext0014.getSpecificErrorMessage(), testURL, this);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			endOfExecuteLoop(false, testURL, threadStep, loopPluginContext);
			return false;
		}
		...
		// --- HTTP REQUEST: Test [15] <- WEB ADMIN Index 66 ---
		String requestProt0015 = "https";
		String requestHost0015 = "www.weightwatchers.com.au";
		int    requestPort0015 = 443;
		String requestFile0015 = "/au/checkout/stylesheets/switch.min.css" +
				"?v=5.0.0-46";
		...
		synchronized (plugin0006)
		{
			...
			((GenericPluginInterface) plugin0006).execute(urlPluginBeforeCallContext0015);			// execute plug-in
		...
		// --- HTTP REQUEST: Test [17] <- WEB ADMIN Index 68 ---
		String requestProt0017 = "https";
		String requestHost0017 = "www.weightwatchers.com";
		int    requestPort0017 = 443;
		String requestFile0017 = "/assets/cmx/au/messages/checkout_switch-min.js";
		...
		// --- HTTP REQUEST: Test [18] <- WEB ADMIN Index 69 ---
		String requestProt0018 = "https";
		String requestHost0018 = "www.weightwatchers.com.au";
		int    requestPort0018 = 443;
		String requestFile0018 = "/au/checkout/api/eligibleOffers/switchData" +
				"?P";
