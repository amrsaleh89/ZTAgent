...
// -----------------------------------------
//
// Source: Twikey_NLBE_1.java
// Date  : 07 Apr 2017 15:01:01 PST
 ...
 * Automatically generated load test program.
 */
public class Twikey_NLBE_1 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36";
	...
	private static final String EXTERNAL_PARAM_FILE = "Twikey_NLBE_1ExtParams.dat";         // input file name for very large parameter values
	...
	private String tokenID = null;                       // var declaration from web admin var handler: scope = per loop
	

	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public Twikey_NLBE_1()
	{
		super();
	}
	 ...
	 */
	public Twikey_NLBE_1(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log("# Page #2: ");
		log("# ---------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: ", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "api1.weightwatchers.be";
		int    requestPort0001 = 443;
		String requestFile0001 = "/signup/v1/member/login";
		String requestContent0001 = 
				"{" + "\r\n" + 
				"  \"username\": \"nocsupport_nlbe\"," + "\r\n" + 
				"  \"password\": \"test1234\"" + "\r\n" + 
				"}";
		String requestHeader0001 = "POST " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: api1.weightwatchers.be\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"Content-Length: " + requestContent0001.length() + "\r\n" + 		// Content-Length: 64
				"Postman-Token: afe1f14a-2018-b8fd-e4c1-fd6070f4ef10\r\n" + 
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "success");		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		// setup var extractor for 'tokenID'
		TextLineTokenExtractorItem lineTokenExtractorItem0001_1 = new TextLineTokenExtractorItem(TextLineTokenExtractorItem.LINE_CONTAINS_PATTERN, "tokenId", 0, "\"", 10);
		...
		{
			// extract var 'tokenID'
			tokenID = lineTokenExtractorItem0001_1.getResultToken();
			log("<<< tokenID = " + tokenID);
		}
		catch (Exception e) { tokenID = null; log(e); }
		if (tokenID == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'tokenID' from text line pattern";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0003 = "api1.weightwatchers.be";
		int    requestPort0003 = 443;
		String requestFile0003 = "/signup/v1/member/enroll/twikey/init" +
				"?v=5.4.0-15";
		String requestContent0003 = 
				"{\"GeneralBillingInfo\":{\"BillingAddress\":{\"AddressLine1\":\"Rue du\",\"AddressLine2\":\"\",\"City\":\"Brussels\",\"StateId\":\"\",\"State\":null,\"CountryId\":\"9000\",\"PostalCode\":\"1210\"},\"ShippingAddress\":{\"AddressLine1\":\"Rue du Progr\u00e8s 9\",\"AddressLine2\":\"\",\"City\":\"Brussels\",\"StateId\":\"\",\"State\":null,\"CountryId\":\"9000\",\"PostalCode\":\"1210\"},\"paymentRequired\":true,\"FirstName\":\"NOC\",\"LastName\":\"NOC\"},\"Phone\":\"3243243243\",\"ProductId\":\"53653\",\"EmailTemplate\":\"SignupOnline\",\"MonthlyPassType\":1}";
		String requestHeader0003 = "POST " + requestFile0003 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Cache-Control: no-cache\r\n" + 
				"Postman-Token: 8e52a701-bfc5-cae1-6310-03733e3847d7\r\n" + 
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 743
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "twikeySignature\":\"cde5bd1edaca6801b9f8cfa1ef2eab52946584857be89d2a4658d220da86df50\",\"twikeyUrl\":\"https://weightwatchers.twikey.com/", "AND", "l=nl");		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("Twikey_NLBE_1", concurrentUsers);
		...
		performanceData.setInfoText("Twikey_NLBE_1");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				Twikey_NLBE_1 simulatedUser = new Twikey_NLBE_1(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
