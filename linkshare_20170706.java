...
// -----------------------------------------
//
// Source: linkshare_20170706.java
// Date  : 06 Jul 2017 14:17:06 PST
 ...
 * Automatically generated load test program.
 */
public class linkshare_20170706 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.11; rv:54.0) Gecko/20100101 Firefox/54.0";
	...
	private static final String EXTERNAL_PARAM_FILE = "linkshare_20170706ExtParams.dat";    // input file name for very large parameter values
	 ...
	 */
	public linkshare_20170706(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "linkshare.domo.com";
		int    requestPort0001 = 443;
		String requestFile0001 = "/auth/index";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Host: linkshare.domo.com\r\n" + 
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 10928
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "This product does not support your browser. Upgrade!");		// Test [1] <- Index 1
		if (!urlCallPassed)
		{
		...
		String requestHost0002 = "linkshare.domo.com";
		int    requestPort0002 = 443;
		String requestFile0002 = "/api/domoweb/auth/login";
		String requestContent0002 = 
				"{\"username\":\"alertsite@linkshare.com\",\"password\":\"cjFzdDMkdDNy\",\"base64\":true}";
		...
		String requestHeader0002 = "POST " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-US,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"X-DOMO-RequestContext: {\"clientToe\":\"QM8UA9O3RR-VDCUN\"}\r\n" + 
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 508
		// content test algorithm: check size +/- 5%, defined content size = 508
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 508, 5);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0003 = "linkshare.domo.com";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 1402
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div class=\"app-navigation dm-bootstrap-placeholder\"");		// Test [3] <- Index 3
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0004 = "linkshare.domo.com";
		int    requestPort0004 = 443;
		String requestFile0004 = "/kpis/getkpiorder" +
				"?pageId=-100000";
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 127
		// content test algorithm: check size +/- 5%, defined content size = 127
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, 127, 5);		// Test [4] <- Index 4
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("linkshare_20170706", concurrentUsers);
		...
		performanceData.setInfoText("linkshare_20170706");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				linkshare_20170706 simulatedUser = new linkshare_20170706(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
