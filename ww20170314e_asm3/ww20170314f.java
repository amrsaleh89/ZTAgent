...
// -----------------------------------------
//
// Source: ww20170314f.java
// Date  : 17 Mar 2017 10:25:32 PST
 ...
 * Automatically generated load test program.
 */
public class ww20170314f extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "ww20170314fExtParams.dat";           // input file name for very large parameter values
	 ...
	 */
	public ww20170314f(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0004 = "www.weightwatchers.com.au";
		int    requestPort0004 = 443;
		String requestFile0004 = "/au/checkout/scripts/vendor.js" +
				"?v=5.0.0-46";
		...
		String requestFile0005 = "/au/checkout/scripts/angulars.js" +
				"?v=5.0.0-46";
		...
		String requestHost0006 = "www.weightwatchers.com";
		int    requestPort0006 = 443;
		String requestFile0006 = "/assets/cmx/au/messages/checkout-signup-min.js";
		...
		String requestHost0007 = "www.weightwatchers.com.au";
		int    requestPort0007 = 443;
		String requestFile0007 = "/au/checkout/";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 7813
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div ng-class=\"hideSpinner\" class=\"spinner-container\"");		// Test [7] <- Index 7
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0008 = "www.weightwatchers.com.au";
		int    requestPort0008 = 443;
		String requestFile0008 = "/au/checkout/stylesheets/checkout.min.css" +
				"?v=5.0.0-46";
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {304}, null, null, 0, 5);		// Test [8] <- Index 8
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0009 = "www.weightwatchers.com.au";
		int    requestPort0009 = 443;
		String requestFile0009 = "/au/checkout/scripts/vendor.js" +
				"?v=5.0.0-46";
		...
		// verify response: status code = 304, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {304}, null, null, 0, 5);		// Test [9] <- Index 9
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0010 = "www.weightwatchers.com";
		int    requestPort0010 = 443;
		String requestFile0010 = "/assets/cmx/au/messages/checkout-signup-min.js";
		...
		// verify response: status code = 304, content type = "APPLICATION/X-JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {304}, "APPLICATION/X-JAVASCRIPT", null, null);		// Test [10] <- Index 10
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0011 = "www.weightwatchers.com.au";
		int    requestPort0011 = 443;
		String requestFile0011 = "/au/checkout/scripts/appCheckout.js" +
				"?v=5.0.0-46";
		...
		// verify response: status code = 304, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {304}, null, null, 0, 5);		// Test [11] <- Index 11
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0012 = "www.weightwatchers.com.au";
		int    requestPort0012 = 443;
		String requestFile0012 = "/au/checkout/scripts/angulars.js" +
				"?v=5.0.0-46";
		...
		// verify response: status code = 304, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: check size +/- 5%, defined content size = 0
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {304}, null, null, 0, 5);		// Test [12] <- Index 12
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0013 = "www.weightwatchers.com.au";
		int    requestPort0013 = 443;
		String requestFile0013 = "/au/checkout/api/plans/csp/1/1/1/0/0";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, null);		// Test [13] <- Index 13
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		ContentTokenExtractorItem contentTokenExtractorItem0013_1 = new ContentTokenExtractorItem(":17611", "\t\n\r \"(),/={}", 14);
		...
		{
			// extract var 'ckval02'
			ckval02 = contentTokenExtractorItem0013_1.getResultToken();
			log("<<< ckval02 = " + ckval02);
		}
		...
		String requestHost0014 = "www.weightwatchers.com.au";
		int    requestPort0014 = 443;
		String requestFile0014 = "/au/checkout/images/background_img2_v3.jpg";
		...
		// verify response: status code = 200, content type = "IMAGE/JPEG", header text fragment = [verification disabled], recorded content size = 182899
		// content test algorithm: check size +/- 5%, defined content size = 182899
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/JPEG", null, 182899, 5);		// Test [14] <- Index 14
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestFile0015 = "/au/checkout/images/favicon-ww-32px.png";
		...
		// verify response: status code = 200, content type = "IMAGE/PNG", header text fragment = [verification disabled], recorded content size = 1151
		// content test algorithm: check size +/- 5%, defined content size = 1151
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/PNG", null, 1151, 5);		// Test [15] <- Index 15
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0017 = "www.weightwatchers.com.au";
		int    requestPort0017 = 443;
		String requestFile0017 = "/au/checkout/icons/checkmark.svg";
		...
		// verify response: status code = 200, content type = "IMAGE/SVG+XML", header text fragment = [verification disabled], recorded content size = 742
		// content test algorithm: check size +/- 5%, defined content size = 742
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "IMAGE/SVG+XML", null, 742, 5);		// Test [17] <- Index 17
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
	...
	boolean executePage_4(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0020 = "api1.weightwatchers.com.au";
		int    requestPort0020 = 443;
		String requestFile0020 = "/signup/v1/member/login/newsLetterOption" +
				"?v=5.0.0-46";
		String requestHeader0020 = "OPTIONS " + requestFile0020 + " HTTP/" + httpProtocolVersion + "\r\n" + 
		...
		String requestHost0021 = "api1.weightwatchers.com.au";
		int    requestPort0021 = 443;
		String requestFile0021 = "/signup/v1/member/login/newsLetterOption" +
				"?v=5.0.0-46";
		...
		ContentTokenExtractorItem contentTokenExtractorItem0021_1 = new ContentTokenExtractorItem("tokenId", "\t\n\r \"(),={}", 2);
		contentTokenExtractor.addTokenExtractorItem(contentTokenExtractorItem0021_1);

		// setup var extractor for 'ckval03'
		ContentTokenExtractorItem contentTokenExtractorItem0021_2 = new ContentTokenExtractorItem("firstName", "\t\n\r \"(),=@{}", 2);
		contentTokenExtractor.addTokenExtractorItem(contentTokenExtractorItem0021_2);

		contentTokenExtractor.extractTokens();
		try
		{
			// extract var 'ckval04'
			ckval04 = contentTokenExtractorItem0021_1.getResultToken();
			log("<<< ckval04 = " + ckval04);
		}
		...
		{
			// extract var 'ckval03'
			ckval03 = contentTokenExtractorItem0021_2.getResultToken();
			log("<<< ckval03 = " + ckval03);
		}
		...
		String requestHost0022 = "www.weightwatchers.com.au";
		int    requestPort0022 = 443;
		String requestFile0022 = "/au/checkout/switch";
		...
		synchronized (plugin0004)
		{
			...
			((GenericPluginInterface) plugin0004).execute(urlPluginBeforeCallContext0022);			// execute plug-in
		...
		synchronized (plugin0002)
		{
			...
			((GenericPluginInterface) plugin0002).execute(urlPluginBeforeCallContext0022);			// execute plug-in
		...
		synchronized (plugin0003)
		{
			...
			((GenericPluginInterface) plugin0003).execute(urlPluginBeforeCallContext0022);			// execute plug-in
		...
		synchronized (plugin0005)
		{
			...
			((GenericPluginInterface) plugin0005).execute(urlPluginBeforeCallContext0022);			// execute plug-in
		...
		LoadtestPluginContext urlPluginContext0022 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		...
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "div ng-class=\"hideSpinner\" class=\"spinner-container\"");		// Test [22] <- Index 22
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			...
			urlPluginContext0022.setUrlPassed(false);
			...
			((GenericPluginInterface) plugin0001).execute(urlPluginContext0022);			// execute plug-in
		...
		// execute plug-in #1 after url call
		urlPluginContext0022.setUrlPassed(urlCallPassed);
		log(">>> execute plug-in #1: GetCookieValue");
		((GenericPluginInterface) plugin0001).setInputParameter(0, ckname05);		// set plug-in input parameter
		((GenericPluginInterface) plugin0001).execute(urlPluginContext0022);			// execute plug-in
		if (((GenericPluginInterface) plugin0001).getLogVector() != null)			// dump plug-in log
			log(((GenericPluginInterface) plugin0001).getLogVector().getLog());
		if (urlPluginContext0022.getSpecificErrorMessage() != null)
		{
			...
			log(urlPluginContext0022.getSpecificErrorMessage());
			performanceData.getPerformanceDataRecord(threadStep).modifyFailureActionType(HttpTestURL.FAILURE_ACTION_HANDLE_AS_ERROR);
			performanceData.setFailed(threadStep, urlPluginContext0022.getSpecificErrorStatusType(), urlPluginContext0022.getSpecificErrorMessage(), testURL, this);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			endOfExecuteLoop(false, testURL, threadStep, loopPluginContext);
			return false;
		}
		...
		String requestHost0023 = "www.weightwatchers.com.au";
		int    requestPort0023 = 443;
		String requestFile0023 = "/au/checkout/stylesheets/switch.min.css" +
				"?v=5.0.0-46";
		...
		LoadtestPluginContext urlPluginBeforeCallContext0023 = new LoadtestPluginContext(prxVersion, prxCharEncoding, this, threadNumber, socketPool, cookieHandler, testURL, threadStep);
		...
		synchronized (plugin0006)
		{
			...
			((GenericPluginInterface) plugin0006).execute(urlPluginBeforeCallContext0023);			// execute plug-in
		...
		String requestHost0024 = "www.weightwatchers.com.au";
		int    requestPort0024 = 443;
		String requestFile0024 = "/au/checkout/scripts/appSwitch.js" +
				"?v=5.0.0-46";
		...
		String requestHost0025 = "www.weightwatchers.com";
		int    requestPort0025 = 443;
		String requestFile0025 = "/assets/cmx/au/messages/checkout_switch-min.js";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, null);		// Test [25] <- Index 25
		if (!urlCallPassed)
		{
			// failure but continue current loop: mark pending transactions as defective and dump URL content to thread log
			transactionHandler.setPendingTransactionsAsDefective();
		...
		String requestHost0026 = "www.weightwatchers.com.au";
		int    requestPort0026 = 443;
		String requestFile0026 = "/au/checkout/api/eligibleOffers/switchData" +
				"?P";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, null);		// Test [26] <- Index 26
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("ww20170314f", concurrentUsers);
		...
		final int MAX_THREAD_STEPS = 23;  // number of URL requests in execute() - modify this value if you add or delete some requests
		...
		performanceData.setInfoText("ww20170314f");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new ww20170314f());
		try
		{
			// define object for plug-in #4 and call plug-in constructor
			Class pluginClass0 = Class.forName("CookieInjector");
			...
			Class pluginClass1 = Class.forName("CookieInjector");
			...
			Class pluginClass2 = Class.forName("CookieInjector");
			...
			Class pluginClass3 = Class.forName("CookieInjector");
			...
			Class pluginClass4 = Class.forName("CookieInjector");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				ww20170314f simulatedUser = new ww20170314f(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
